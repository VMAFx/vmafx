...
// Package pyjsonstrict renders Go values the way vmaftune.jsonio.dumps_strict
// does: non-finite floats become JSON null (NaNAsNull) so the payload stays
// valid RFC 8259, or bare NaN / Infinity tokens (NaNAsToken) matching a plain
// json.dumps with allow_nan=True.
//
// This is deliberately a separate package from internal/pyjson, which mirrors
// json.dumps(indent=..., sort_keys=...). The two encoders mirror two different
// Python entry points with different non-finite handling, and merging them
// would mean one package silently answering to two output contracts. They were
// written independently by the benchmark/encode-profile port (this one) and the
// ML-driven port (internal/pyjson); both are kept because both contracts are
// exercised by real subcommands.
