...
- The `clinfo` apt layer is merged into the NEO `.deb` install layer, removing
...
- `libvmaf/build` (Meson build directory including CUDA PTX, SYCL AOT fat
  binaries, HIP `.co` objects) is removed in the same `RUN` step as
