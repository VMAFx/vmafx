Fix 7 reproducible test failures introduced by the recent PR train:
...
- `test_pic_preallocation`: `threaded_extract_batch_func()` in `libvmaf.c`
  now calls `vmaf_picture_unref()` before `memset()` when clearing
  `fex->prev_ref` after extraction for `VMAF_FEATURE_EXTRACTOR_PREV_REF`
  extractors.  The previous bare `memset()` leaked the extra refcount bump
  introduced by the PREV_REF SWAP in `feature_extractor.cpp`, causing the
  preallocated picture pool to exhaust after `pool_size` frames and blocking
  `vmaf_fetch_preallocated_picture()` forever.  The same fix is applied to
  the (currently dead-code) `threaded_extract_func()` for consistency.
