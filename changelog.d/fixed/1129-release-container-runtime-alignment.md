- Release containers now keep their build and runtime ABIs aligned: CPU and
  ...
  signature-verifying smokes replace success-masked checks. Operator and node
  images inject that release tag into the shared Go version package and expose
  a non-blocking `--version` check for runtime verification.
