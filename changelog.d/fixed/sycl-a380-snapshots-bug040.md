...
- `core/src/libvmaf.c` (`read_pictures_frame_cleanup`): fix host-buffer DMA lifetime
  ...
  in-flight GPU data. Added `vmaf_sycl_wait_last_upload(vmaf->sycl.state)` before
  `vmaf_picture_unref` under `#ifdef HAVE_SYCL` to drain the last DMA event without
  blocking GPU compute on `combined_queue`. Verified with 20 consecutive 4K runs on
  Arc A380: vmaf=90.747255 adm2=1.013590 (bit-exact, vs CPU delta 1.6e-05 ≤ tolerance).
  `testdata/test_sycl_4k_repeat_determinism.py`: add pytest harness asserting consecutive
  4K SYCL runs are bit-exact (≤ 1e-6); skips gracefully without 4K YUV fixtures.
