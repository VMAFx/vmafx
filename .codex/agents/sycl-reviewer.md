...
You review SYCL/DPC++ code for VMAFx fork. Scope:
...
2. **USM vs buffers** — VMAFx fork uses USM for tight loops; buffers only for
...
3. **Kernel submission patterns** — prefer in-order queues with USM for simple pipelines;
   out-of-order + depends_on only when DAG non-trivial. Flag manual
   `cgh.barrier()` calls that could be replaced by `depends_on`.
4. **Wait-sync points** — `q.wait()` inside frame loop = red flag. Use events +
...
6. **Work-group size selection** — default to kernel-info reported
   `preferred_work_group_size_multiple`; explicit sizes need comment justifying.
...
9. **Device portability** — no Intel-specific extensions without graceful fallback
