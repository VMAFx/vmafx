...
from vmaf.core.train_test_model import (
    ...
    ResidueBootstrapLibsvmNusvrTrainTestModel,
...
class BootstrapTrainTestModelTest(MyTestCase):
    ...
    def test_train_predict_bootstrap_libsvmnusvr(self):
        ...
        self.assertAlmostEqual(
            model.evaluate_stddev(xs)["mean_stddev"], 0.39703824367678103, places=2
        )
        self.assertAlmostEqual(
            model.evaluate_stddev(xs)["mean_ci95_low"], 3.3984211902006627, places=2
        )
        self.assertAlmostEqual(
            model.evaluate_stddev(xs)["mean_ci95_high"], 4.753825468723706, places=2
        )

        model = BootstrapLibsvmNusvrTrainTestModel({"norm_type": "clip_0to1"}, None)
        model.train(xys)
        ...
        self.assertAlmostEqual(
            model.evaluate_stddev(xs)["mean_stddev"], 0.34509843867201334, places=2
        )
        self.assertAlmostEqual(
            model.evaluate_stddev(xs)["mean_ci95_low"], 3.5003674768667943, places=2
        )
        self.assertAlmostEqual(
        ...
        self.assertAlmostEqual(
            model.evaluate_stddev(xs)["mean_stddev"], 0.3510958063355727, places=2
        )
        self.assertAlmostEqual(
            model.evaluate_stddev(xs)["mean_ci95_low"], 3.479040231431352, places=2
        )
        self.assertAlmostEqual(
            model.evaluate_stddev(xs)["mean_ci95_high"], 4.758095707489271, places=2
        )

        model = BootstrapLibsvmNusvrTrainTestModel({"norm_type": "clip_minus1to1"}, None)
        model.train(xys)
        ...
        self.assertAlmostEqual(
            model.evaluate_stddev(xs)["mean_stddev"], 0.37890490520445996, places=2
        )
        self.assertAlmostEqual(
            model.evaluate_stddev(xs)["mean_ci95_low"], 3.395027628418071, places=2
        )
        self.assertAlmostEqual(
            model.evaluate_stddev(xs)["mean_ci95_high"], 4.750492595014303, places=2
        )
        ...
        self.assertAlmostEqual(
            model.evaluate_stddev(xs)["mean_stddev"], 0.3505327957559016, places=2
        )
        self.assertAlmostEqual(
            model.evaluate_stddev(xs)["mean_ci95_low"], 3.6104498263465024, places=2
        )
        self.assertAlmostEqual(
            model.evaluate_stddev(xs)["mean_ci95_high"], 4.617611259682587, places=2
        )
    ...
    def test_train_save_load_predict_bootstrap_libsvmnusvr(self):
        ...
        self.assertAlmostEqual(
            self.model.evaluate_bagging(xs, ys)["RMSE"], 0.6696474832672723, places=4
        )
        self.assertAlmostEqual(
            self.model.evaluate_stddev(xs)["mean_stddev"], 0.39703824367678103, places=2
        )
        self.assertAlmostEqual(
            self.model.evaluate_stddev(xs)["mean_ci95_low"], 3.3984211902006627, places=2
        )
        self.assertAlmostEqual(
            self.model.evaluate_stddev(xs)["mean_ci95_high"], 4.753825468723706, places=2
        )
        ...
        self.assertAlmostEqual(
            loaded_model.evaluate_bagging(xs, ys)["RMSE"], 0.6696478863129723, places=4
        )
        self.assertAlmostEqual(
            loaded_model.evaluate_stddev(xs)["mean_stddev"], 0.3970382109813205, places=2
        )
        self.assertAlmostEqual(
            loaded_model.evaluate_stddev(xs)["mean_ci95_low"], 3.398421319902801, places=2
        )
        self.assertAlmostEqual(
            loaded_model.evaluate_stddev(xs)["mean_ci95_high"], 4.753825575324514, places=2
        )
    ...
    def test_train_across_model_stats_bootstraplibsvmnusvr(self):
        ...
        stats = self.model.get_stats(
            ys_label, ys_label_pred, ys_label_pred_all_models=ys_label_pred_all_models
        )
        ...
        # check dimensions
        assert (
            len(stats["SRCC_across_model_distribution"]) == np.shape(ys_label_pred_all_models)[0]
            and len(stats["PCC_across_model_distribution"]) == np.shape(ys_label_pred_all_models)[0]
            and len(stats["RMSE_across_model_distribution"])
            == np.shape(ys_label_pred_all_models)[0]
        )
        ...
        assert (
            "SRCC_across_model_distribution" not in stats_not_across_model
    ...
    def test_train_predict_bootstrap_randomforest(self):
        ...
        self.assertAlmostEqual(
            model.evaluate_stddev(xs)["mean_stddev"], 0.35136642421562986, places=2
        )
        self.assertAlmostEqual(
            model.evaluate_stddev(xs)["mean_ci95_low"], 3.4185833333333333, places=1
        )
        self.assertAlmostEqual(
            model.evaluate_stddev(xs)["mean_ci95_high"], 4.6789999999999985, places=1
        )

        model = BootstrapSklearnRandomForestTrainTestModel(
            {"norm_type": "clip_0to1", "n_estimators": 10, "random_state": 0}, None
        )
        model.train(xys)
        self.assertAlmostEqual(model.evaluate(xs, ys)["RMSE"], 0.15634392282439, places=4)
        self.assertAlmostEqual(
            model.evaluate_bagging(xs, ys)["RMSE"], 0.16538552470948875, places=2
        )
        self.assertAlmostEqual(
            model.evaluate_stddev(xs)["mean_stddev"], 0.3539523612820203, places=2
        )
        self.assertAlmostEqual(
            model.evaluate_stddev(xs)["mean_ci95_low"], 3.4091111111111108, places=2
        )
        self.assertAlmostEqual(
        ...
        model = BootstrapSklearnRandomForestTrainTestModel(
            {"norm_type": "clip_minus1to1", "n_estimators": 10, "random_state": 0}, None
        )
        model.train(xys)
        self.assertAlmostEqual(model.evaluate(xs, ys)["RMSE"], 0.15634392282438947, places=4)
        self.assertAlmostEqual(
            model.evaluate_bagging(xs, ys)["RMSE"], 0.16634325875304712, places=2
        )
        self.assertAlmostEqual(
            model.evaluate_stddev(xs)["mean_stddev"], 0.35470969070245834, places=2
        )
        self.assertAlmostEqual(
            model.evaluate_stddev(xs)["mean_ci95_low"], 3.405222222222222, places=2
        )
        self.assertAlmostEqual(
        ...
        self.assertAlmostEqual(
            model.evaluate_bagging(xs, ys)["RMSE"], 0.16935240638616547, places=3
        )
        self.assertAlmostEqual(
            model.evaluate_stddev(xs)["mean_stddev"], 0.3538762838023098, places=2
        )
        self.assertAlmostEqual(
            model.evaluate_stddev(xs)["mean_ci95_low"], 3.402638888888889, places=2
        )
        self.assertAlmostEqual(
            model.evaluate_stddev(xs)["mean_ci95_high"], 4.679027777777778, places=2
        )
    ...
    def test_train_save_load_predict_bootstrap_randomforest(self):
        ...
        self.model = BootstrapSklearnRandomForestTrainTestModel(
            {"norm_type": "normalize", "n_estimators": 10, "random_state": 0}, None
        )
        self.model.train(xys)
        ...
        loaded_model = BootstrapSklearnRandomForestTrainTestModel.from_file(
            self.model_filename, None
        )

        self.assertAlmostEqual(self.model.evaluate(xs, ys)["RMSE"], 0.17634739353518517, places=4)
        self.assertAlmostEqual(
            self.model.evaluate_bagging(xs, ys)["RMSE"], 0.1734023350765026, places=2
        )
        self.assertAlmostEqual(
            self.model.evaluate_stddev(xs)["mean_stddev"], 0.35136642421562986, places=2
        )
        self.assertAlmostEqual(
            self.model.evaluate_stddev(xs)["mean_ci95_low"], 3.4185833333333333, places=1
        )
        self.assertAlmostEqual(
            self.model.evaluate_stddev(xs)["mean_ci95_high"], 4.6789999999999985, places=1
        )

        self.assertAlmostEqual(loaded_model.evaluate(xs, ys)["RMSE"], 0.17634739353518517, places=4)
        self.assertAlmostEqual(
            loaded_model.evaluate_bagging(xs, ys)["RMSE"], 0.1734023350765026, places=2
        )
        self.assertAlmostEqual(
            loaded_model.evaluate_stddev(xs)["mean_stddev"], 0.35136642421562986, places=2
        )
        self.assertAlmostEqual(
            loaded_model.evaluate_stddev(xs)["mean_ci95_low"], 3.4185833333333333, places=1
        )
        self.assertAlmostEqual(
            loaded_model.evaluate_stddev(xs)["mean_ci95_high"], 4.6789999999999985, places=1
        )
    ...
    def test_train_save_load_predict_residue_bootstrap_libsvmnusvr(self):
        ...
        loaded_model = ResidueBootstrapLibsvmNusvrTrainTestModel.from_file(
            self.model_filename, None
        )

        self.assertAlmostEqual(self.model.evaluate(xs, ys)["RMSE"], 0.6226308662005923, places=4)
        self.assertAlmostEqual(
            self.model.evaluate_bagging(xs, ys)["RMSE"], 0.955069756575906, places=4
        )
        self.assertAlmostEqual(
            self.model.evaluate_stddev(xs)["mean_stddev"], 0.3437214386264522, places=2
        )
        self.assertAlmostEqual(
            self.model.evaluate_stddev(xs)["mean_ci95_low"], 3.4674811219156174, places=2
        )
        self.assertAlmostEqual(
            self.model.evaluate_stddev(xs)["mean_ci95_high"], 4.671073568422049, places=2
        )

        self.assertAlmostEqual(loaded_model.evaluate(xs, ys)["RMSE"], 0.6226313987163097, places=4)
        self.assertAlmostEqual(
            loaded_model.evaluate_bagging(xs, ys)["RMSE"], 0.9550696941149013, places=4
        )
        self.assertAlmostEqual(
            loaded_model.evaluate_stddev(xs)["mean_stddev"], 0.3437213993262196, places=2
        )
        self.assertAlmostEqual(
            loaded_model.evaluate_stddev(xs)["mean_ci95_low"], 3.4674811883034935, places=2
        )
        self.assertAlmostEqual(
            loaded_model.evaluate_stddev(xs)["mean_ci95_high"], 4.671073598975557, places=2
        )
    ...
    def test_train_save_load_predict_residue_bootstrap_randomforest(self):
        ...
        self.model = ResidueBootstrapRandomForestTrainTestModel(
            {"norm_type": "normalize", "n_estimators": 10, "random_state": 0}, None
        )
        self.model.train(xys)
        ...
        loaded_model = ResidueBootstrapRandomForestTrainTestModel.from_file(
            self.model_filename, None
        )

        self.assertAlmostEqual(self.model.evaluate(xs, ys)["RMSE"], 0.17634739353518517, places=4)
        self.assertAlmostEqual(
            self.model.evaluate_bagging(xs, ys)["RMSE"], 0.2731966696149454, places=2
        )
        self.assertAlmostEqual(
            self.model.evaluate_stddev(xs)["mean_stddev"], 0.2176592325445535, places=2
        )
        self.assertAlmostEqual(
            self.model.evaluate_stddev(xs)["mean_ci95_low"], 3.621466666666666, places=2
        )
        self.assertAlmostEqual(
            self.model.evaluate_stddev(xs)["mean_ci95_high"], 4.4107916666666656, places=2
        )

        self.assertAlmostEqual(loaded_model.evaluate(xs, ys)["RMSE"], 0.17634739353518517, places=4)
        self.assertAlmostEqual(
            loaded_model.evaluate_bagging(xs, ys)["RMSE"], 0.2731966696149454, places=2
        )
        self.assertAlmostEqual(
            loaded_model.evaluate_stddev(xs)["mean_stddev"], 0.2176592325445535, places=2
        )
        self.assertAlmostEqual(
            loaded_model.evaluate_stddev(xs)["mean_ci95_low"], 3.621466666666666, places=2
        )
        self.assertAlmostEqual(
            loaded_model.evaluate_stddev(xs)["mean_ci95_high"], 4.4107916666666656, places=2
        )
