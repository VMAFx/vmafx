...
class TrainTestModelTest(MyTestCase):
    ...
    def test_get_xs_ys(self):
        ...
        self.assertAlmostEqual(
            np.mean(xs["Moment_noref_feature_1st_score"]), 128.26146851380497, places=4
        )
        self.assertEqual(len(xs["Moment_noref_feature_var_score"]), 3)
        self.assertAlmostEqual(
            np.mean(xs["Moment_noref_feature_var_score"]), 1569.2395085695462, places=4
        )

        xs = TrainTestModel.get_xs_from_results(self.features)
        self.assertEqual(len(xs["Moment_noref_feature_1st_score"]), 9)
        self.assertAlmostEqual(
            np.mean(xs["Moment_noref_feature_1st_score"]), 111.59099599173773, places=4
        )
        self.assertEqual(len(xs["Moment_noref_feature_var_score"]), 9)
        self.assertAlmostEqual(
            np.mean(xs["Moment_noref_feature_var_score"]), 1806.8620377229011, places=4
        )

        ys = TrainTestModel.get_ys_from_results(self.features, [0, 1, 2])
        expected_ys = {"label": np.array([2.5, 3.9, 5.0]), "content_id": np.array([0, 1, 2])}
    ...
    def test_train_predict_libsvmnusvr(self):
        ...
        model = LibsvmNusvrTrainTestModel({"norm_type": "clip_minus1to1"}, None)
        model.train(xys)
        result = model.evaluate(xs, ys)
    ...
    def test_train_predict_linearregression(self):
        ...
        xs = SklearnLinearRegressionTrainTestModel.get_xs_from_results(
            self.features, [0, 1, 2, 3, 4, 5]
        )
        ys = SklearnLinearRegressionTrainTestModel.get_ys_from_results(
            self.features, [0, 1, 2, 3, 4, 5]
        )
        xys = SklearnLinearRegressionTrainTestModel.get_xys_from_results(
            self.features, [0, 1, 2, 3, 4, 5]
        )
    ...
    def test_train_logistic_fit_5PL(self):
        xs = Logistic5PLRegressionTrainTestModel.get_xs_from_results(
            self.features, [0, 1, 2, 3, 4, 5], features=["Moment_noref_feature_1st_score"]
        )
        ys = Logistic5PLRegressionTrainTestModel.get_ys_from_results(
            self.features, [0, 1, 2, 3, 4, 5]
        )
