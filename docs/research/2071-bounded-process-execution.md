# Repository subprocess boundary audit
...
explicit allowlist, resolves it before launch, rejects malformed or oversized
arguments and environments, bounds captured output, closes unused stdin, and
enforces a wall-clock deadline. On POSIX, each child owns a new session and
cleanup terminates the process group. The API has synchronous and asynchronous
forms so an async caller cannot accidentally block its active event loop.
...
and argument values, output flooding, timeouts, descendant cleanup, and the
case where a session leader exits before its child. Consumer tests drive real
