...
rather than `#include`-ing the cbindgen-generated header (per AGENTS.md invariant 5:
"cbindgen-generated headers are not committed"). The three declarations in
`tad_rust.c` are:
...
`lazycell 1.3.0` is pulled transitively by `bindgen 0.69.5` (build-dependency only —
not present at runtime). The advisory reports that `LazyCell::get_or_try_insert_with`
...
The workspace `Cargo.lock` (version 4) was regenerated as part of this audit and is
tracked under the current branch. Status:
