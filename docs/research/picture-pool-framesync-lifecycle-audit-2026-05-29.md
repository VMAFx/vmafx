...
### Finding A — PREV_REF aliased without refcount in `read_pictures_dispatch_one` (severity: HIGH)
...
### Finding B — `vmaf_gpu_picture_pool_close` frees pictures without synchronisation (severity: MEDIUM)
...
### Finding E — `pool_preallocate_pictures` error path unrefs pictures through `vmaf_picture_unref` whose priv is NULL (severity: MEDIUM)
...
| ID | Severity | Class | Fixed in this PR |
|----|----------|-------|-----------------|
| A  | HIGH     | Refcount inversion / UAF window | Yes |
| B  | MEDIUM   | GPU in-flight free on SYCL teardown | No (tracked) |
| C  | LOW      | Framesync list footprint | No (cosmetic) |
| D  | LOW      | Pool close deadlock on leak | No (tracked) |
| E  | MEDIUM   | Error-path buffer leak in pool_preallocate | Yes |
