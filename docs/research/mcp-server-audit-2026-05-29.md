...
## Finding 5 — `subsample` parameter silently dropped in `vmaf_score_encoded` (line 1720)
...
## Finding 7 — Schema/surface drift: `vmaf_score` accepts `bitdepth=16` but libvmaf does not
...
| 4 | **High** | Schema/surface drift (wrong path → empty result) | server.py | 858, 86 |
