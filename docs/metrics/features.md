...
| `adm_csf_mode`           | `csf`  | int    | `0`       | `0–3`       | Contrast-sensitivity-function model: `0` Watson97 (upstream-canonical), `1` Barten, `2` Barten/Watson blend, `3` Barten/Watson blend (MAE-fitted). The default model `vmaf_v1.0.16_3d0h` requests `2`. Configurations whose scale-0 fixed-point factors exceed the 16-bit budget (`factor1 * 2^21 >= 65536` or `factor2 * 2^23 >= 65536`, e.g. mode 1 at default 1080p/3H) are rejected with `-EINVAL`. |
