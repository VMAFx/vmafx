...
The change covers:
- `adm_csf.cu`: `i4_adm_csf_kernel<>` (int32 path) and `adm_csf_kernel<>` (int16 path)
...
- req: "Apply the same F3 fix (raw __restrict__ pointer extraction + __ldg() on
  read-only loads) to the INLINE header — that's the path that actually executes."
