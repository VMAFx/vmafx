...
- **Date**: 2026-05-29
- **Deciders**: lusoris
- **Tags**: `correctness`, `picture-pool`, `framesync`, `refcount`, `gpu`, `sycl`, `audit`, `fork-local`
...
|--------|------|------|----------------|
| Fix all five findings in one PR | Maximises coverage | SYCL sync-callback requires larger API change; D is cosmetic | Would block the HIGH-severity fix on a design discussion |
| Fix only Finding A | Minimal risk | Leaves the pool-preallocate leak (E) unfixed | E is equally mechanical and should land together |
| No fix, document-only | Zero code risk | HIGH-severity UAF window stays open | Not acceptable for a correctness repo |
