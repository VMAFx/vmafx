# ADR-0764: psnr_hvs CUDA kernel — `__ldg()` + `__restrict__` pointer extraction + `__launch_bounds__(64)`
...
- Source: req — "Apply the F3 fix (`__restrict__` raw pointers + `__ldg()`) to `psnr_hvs` kernel … Mirror PR #93's pattern."
