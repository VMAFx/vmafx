...
4. **libvmaf meson build directory retained** (Stage 3): after `ninja install`,
   the `libvmaf/build` directory containing CUDA PTX/SASS intermediates, SYCL
...
1. **Merge the `clinfo` apt layer** into the NEO `.deb` install layer: install
   ...
   download and install the NEO `.deb` packages. The apt-list cleanup happens
   once at the end of the merged layer.
...
4. **Remove `libvmaf/build`** in the same `RUN` step as `ninja install`, after
   the install completes. The installed shared library and headers under
   `/usr/local/` are retained; only the meson build directory is removed.
...
- **Positive**: Removes several GB from the final image (FFmpeg build objects
  are typically 2-5 GB; libvmaf/build CUDA+SYCL+HIP intermediates add another
  1-3 GB; pip cache is typically 200-600 MB depending on transitive deps).
  Merging the apt layer eliminates one redundant metadata download during
...
- **Neutral**: The `clingo`/`sycl-ls`/`rocminfo` probe in Stage 2 still runs
  after the merged layer, as before. Layer cache keys are unchanged for the
  NEO ARG pins.
