...
| `ffmpeg-patches-apply-check` | pre-push | Replay every patch in `ffmpeg-patches/series.txt` cumulatively with `git am --3way` against a disposable FFmpeg `n9.0.2` checkout. Backed by `scripts/ci/ffmpeg-patches-check.sh`. |
