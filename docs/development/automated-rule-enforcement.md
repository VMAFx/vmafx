...
| `semgrep-local` | pre-commit | Project-local rules from `.semgrep.yml` (`--error` exit code on match). Standard rule packs (`p/cert-c-strict`, `p/cwe-top-25`) still run in CI only. The vendored JSON code (`core/src/pdjson.{c,h}`, `core/src/mcp/3rdparty/cJSON/`) is in scope: the banned-function rule is what stops a re-vendor from bringing `sprintf` / `strcpy` back. |
