...
| [`delivery_mitm`](https://www.bestpractices.dev/en/criteria/0#delivery_mitm) | MUST | Evidence | Source delivery uses [GitHub HTTPS](https://github.com/VMAFx/vmafx); future release-asset delivery must retain HTTPS and verified provenance where promised. |
| [`delivery_unsigned`](https://www.bestpractices.dev/en/criteria/0#delivery_unsigned) | MUST | Unknown | Verify all supported acquisition paths; HTTPS release configuration alone does not prove no path accepts an unauthenticated hash fetched over HTTP. |
