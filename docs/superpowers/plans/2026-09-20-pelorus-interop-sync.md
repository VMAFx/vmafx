...
**Execution status (2026-09-20):** All checklist items completed locally. The
canonical GCC 15 / clang-tidy 22 CPU ratchet matches its normalized debt
baseline: 307 selected translation units, 750 warnings, zero uncited `NOLINT`s,
and zero compile failures. A workstation GCC 16 measurement's five additional
diagnostics are recorded in the research digest rather than misreported as a
canonical failure. Hosted CI was intentionally not started because this task
forbids pushing the branch or opening a pull request.
...
- [x] Confirm the worktree is clean, branch is `fix/pelorus-interop-sync-v022`, and `HEAD == origin/master == 371ff5891ad43b6d8072d9fac132349ee3ddaaa9`.
- [x] Record the exact upstream delta with:
...
- [x] Apply only v0.2.2's `test_misaligned_blob_base()` and `test_unaligned_header_size()` fixture additions, preserving the existing VMAFx-authored header/include rewrite.
- [x] Configure and build an isolated UBSan target against the still-old parser:
...
- [x] Do not commit the failing intermediate state.
...
- [x] Port `libpelorus/src/interop.c` exactly: build pack headers/directories in aligned locals, `memcpy` them to/from the byte image, and reject a non-8-byte-aligned `header_size` with `PEL_ERR_ABI`.
- [x] Update the vendored library version constants to `0.2.2`; leave `PELORUS_ABI_MAJOR=1` and `PELORUS_ABI_MINOR=3` byte-for-byte unchanged.
- [x] Restore the complete fixture body to the v0.2.2 source, apart from the existing include rewrite. Remove PR #1351's VMAFx-only `NOLINTBEGIN`/`NOLINTEND` band and `(void)` return-value casts.
- [x] Update every vendored banner to the full pin `93bef1206d68d9e09024c08a12732fb8e77b9b16` so provenance is unambiguous.
- [x] Rebuild and rerun the UBSan regression command from Task 1.

  Expected GREEN: `test_pelorus_interop` passes with no alignment diagnostic.
- [x] Configure and run a normal focused build:
...
- [x] Commit the parser and exact fixture together:
...
- [x] Add a tidy-ratchet unit test that puts `core/src/interop/pelorus_interop.c`, `core/test/test_pelorus_interop.c`, and an ordinary C file into a compile database. Assert only the ordinary file is selected and legacy baseline entries for the exact mirror are ignored consistently.
- [x] Run:
...
- [x] Add one explicit exact-vendor predicate covering `core/src/interop/pelorus_*.c`, `core/include/libvmaf/pelorus/**`, and `core/test/test_pelorus_interop.c`. Use it both when selecting translation units and when normalizing pre-existing baseline measurements, so all lane baselines remain generated artifacts and need no hand edits.
- [x] Extend `.pre-commit-config.yaml` and both `Makefile` format selectors to exclude the exact shared fixture as well as the already-excluded vendored sources/headers.
- [x] Add a hermetic shell fixture for `scripts/sync-pelorus-interop.sh` that proves a non-Git checkout and a Git checkout missing the pinned object fail closed rather than silently checking working-tree files.
- [x] Change `PELORUS_VENDOR_SHA` to the full v0.2.2 commit and remove the working-tree fallback. A valid checkout may be on any branch/HEAD only if `git cat-file -e "$PELORUS_VENDOR_SHA^{commit}"` succeeds; all source reads come from `git show` of that object.
- [x] Update the script's maintenance policy: re-pin for reviewed ABI additions and released parser correctness/security fixes, even when ABI major/minor do not change.
- [x] Add the shell fixture to the required Pre-Commit hook set, and add a step to the existing `Pre-Commit` job in `.github/workflows/lint-and-format.yml` that checks out `VMAFx/pelorus` at the script's exact pin and runs the default drift/conformance check.
- [x] Run the tooling tests and exact drift check:
...
- [x] Commit the policy/tooling/CI unit:
...
- [x] Add the cross-repo rebase note and an invariant to `core/test/AGENTS.md`: the fixture body is exact Pelorus source; lint/format scope changes belong in VMAFx tooling, never in the fixture.
- [x] Add and render the changelog fragment using the repository's changelog renderer.
- [x] Run Markdown/repository doc checks on every changed document.
- [x] Commit the documentation and release artifacts:
...
- [x] Run the focused normal test and the default mirror check again.
- [x] Configure a fresh ASan build and run the focused test:
...
- [x] Run the fresh UBSan build from Task 1 with `UBSAN_OPTIONS=halt_on_error=1`.
- [x] Run the relevant script/tidy tests, ShellCheck/shfmt, YAML/pre-commit hooks, and a CPU tidy-ratchet measurement.
- [x] Run the repository-required aggregate command:

  ```bash
  make verify-all
  ```

- [x] Inspect `git diff --check origin/master..HEAD`, `git status --short`, the commit sequence, and `git diff origin/master..HEAD` for accidental fixture or ABI drift.
- [x] Verify directly that de-bannered vendored files match Pelorus v0.2.2 and that `PELORUS_ABI_MAJOR/MINOR` remain `1/3`.
- [x] Record any environment-only unavailable gate as a concern with the exact command/error; do not claim it passed.
- [x] Do not push or open a PR. Report `DONE`, `DONE_WITH_CONCERNS`, or `BLOCKED` with commit SHAs, commands/results, and remaining risks.
