...
path collections; and the opaque-type fallback in normalise_manifest_value.
"""
...
class _Opaque:
    """A value that is neither a JSON scalar nor a Mapping/Sequence/Path."""
...
def test_normalise_manifest_value_falls_back_to_str_for_opaque() -> None:
    """Unrecognised types fall through to ``str(value)`` (the opaque branch)."""
...
def test_normalise_manifest_value_list_recurses() -> None:
    """List/tuple branch recurses element-wise."""
...
def test_describe_path_handles_directory(tmp_path: Path) -> None:
    ...
    info = describe_path(sub, repo_root=tmp_path)
...
def test_describe_paths_recurses_into_path_sequence(tmp_path: Path) -> None:
    """A list of Paths is described element-wise as a JSON list."""
    a = tmp_path / "a.txt"
...
# ----------------------- write_manifest_json + write_run_manifest
