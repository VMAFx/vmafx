...
def test_write_parquet_atomic_cleans_up_temp_on_failure(
    ...
    # The original write_parquet_atomic delegates to df.to_parquet; patch it
    # at the method level so the atomic-rename + cleanup path is exercised.
...
def test_write_parquet_atomic_kwargs_forwarded(tmp_path: Path) -> None:
    """Extra kwargs like compression are forwarded to the underlying writer."""
    df = pd.DataFrame({"x": [1, 2]})
    out = tmp_path / "snappy.parquet"

    write_parquet_atomic(df, out, compression="snappy", index=False)

    assert out.is_file()
    assert pd.read_parquet(out)["x"].tolist() == [1, 2]
