...
- **`ai/tests/` script loaders go through `conftest.py`.** Tests under `ai/tests/` that need to dynamically load a standalone `ai/scripts/*.py` (or any other top-level file under `ai/`) call the shared `load_ai_script(rel_path)` / `load_ai_module(rel_path)` helpers from `ai/tests/conftest.py` instead of re-inlining the `importlib.util.spec_from_file_location → module_from_spec → exec_module` recipe. Both helpers register the loaded module in `sys.modules` (so dataclass/pickle/intra-script imports keep working) and accept an optional `name=` override for tests that need to pin a specific `sys.modules` key. This invariant is rebase-insensitive — upstream Netflix/vmaf has no `ai/` package, so there is no merge-conflict surface; the rule exists to keep the fork's test preamble out of every new test file.
