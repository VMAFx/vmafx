...
// Ported subcommands (compare, ladder, report, corpus, sidecar) carry their
...
func newRoot(version string) *clikit.Root {
	root := clikit.New("vmafx-tune-go",
		"vmafx-tune-go — Go port of vmaf-tune (compare/ladder/report/corpus/sidecar)")
...
Fully ported subcommands:
  ...
  sidecar     Local on-host predictor sidecar (status / predict / record /
              batch-record)
