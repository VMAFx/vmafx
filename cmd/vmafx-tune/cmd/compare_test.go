...
func TestStubSubcommands(t *testing.T) {
	...
	// "ladder" and "report" are now ported subcommands (Stages 2 and 4), and
	// "fast" is ported too (its --smoke path runs end to end) — they exit
	// non-zero only when required flags are missing, not as stubs.
	for _, sub := range []string{"tune-per-shot", "corpus", "benchmark", "auto", "sidecar"} {
		sub := sub
