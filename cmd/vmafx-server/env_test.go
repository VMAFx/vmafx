...
//
// cmd/vmafx-server/env_test.go — VMAFX_ env-contract regression guards.
//
...
// grpc.max_recv_size, grpc.max_send_size) only bind if serverEnvOptions declares
...
// CompoundKey set (the four underscore-bearing grpc leaves). If golusoris'
// grpc.Options grows another underscore leaf, this reminds us to register it.
func TestServerEnvOptionsContract(t *testing.T) {
	t.Parallel()
	opts := serverEnvOptions(true)
	...
	if !opts.Watch {
		t.Error("serverEnvOptions(true).Watch = false, want true")
	}
	if serverEnvOptions(false).Watch {
		t.Error("serverEnvOptions(false).Watch = true, want false")
	}
...
// TestServerEnvOptionsBindGrpcKeys is the end-to-end regression guard: the
// VMAFX_GRPC_* env vars must resolve onto the dotted grpc.* leaf keys through a
// real config.Config built with serverEnvOptions().
func TestServerEnvOptionsBindGrpcKeys(t *testing.T) {
	t.Setenv("VMAFX_GRPC_MAX_RECV_SIZE", "8388608")
	t.Setenv("VMAFX_GRPC_MAX_SEND_SIZE", "8388608")
	...
	if got := cfg.Int("grpc.max_send_size"); got != 8388608 {
		t.Errorf("grpc.max_send_size = %d, want 8388608", got)
	}
