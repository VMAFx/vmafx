 ...
 *
 * The whole fixture storage -- frames, padding rows, stride padding and
 * scratch -- has to end up byte-identical to the scalar run, which doubles as
 * the guard band: a store the scalar kernel never makes fails the test even
 * when num/den agree. Heights sweep from 7 up through the 17-row extractor
 * minimum and a few just above it. The 32K-entry logarithm table is generated
 * once and each geometry reuses its fixtures across the depth/scale/pattern
 * sweep, so gcov instrumentation does not turn setup into the dominant cost.
 */
#include <math.h>
#include <stdint.h>
#include <stdio.h>
                             ...
                             unsigned scale, unsigned pattern)
{
    ...
    if (bpc == 8u && scale == 0u) {
        ...
        vif_statistic_8_avx2(&simd->state, simd_result, simd_result + 1, width, height);
    } else {
        vif_statistic_16(&scalar->state, scalar_result, scalar_result + 1, width, height, (int)bpc,
                         (int)scale);
        vif_statistic_16_avx2(&simd->state, simd_result, simd_result + 1, width, height, (int)bpc,
                              (int)scale);
    }
