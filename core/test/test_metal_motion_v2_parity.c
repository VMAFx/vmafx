 ...
 *
 * at frame index 0 matches within the places=4 (1e-4) tolerance from
...
/* ------------------------------------------------------------------ */
/* CPU path — run `motion_v2` for NUM_FRAMES frames, return
 * motion2_v2_score at frame index 0.                                  */
/* ------------------------------------------------------------------ */
static char *run_cpu_motion_v2(double *out_score)
{
    int err = 0;
    ...
    err = vmaf_feature_score_at_index(vmaf, "VMAF_integer_feature_motion2_v2_score", out_score, 0u);
    mu_assert("CPU: vmaf_feature_score_at_index(motion2_v2, idx=0) failed", !err);
...
/* ------------------------------------------------------------------ */
/* Metal path — run `motion_v2_metal` for NUM_FRAMES frames, return
 * motion2_v2_score at frame index 0. NaN out-score = "skip".          */
/* ------------------------------------------------------------------ */
static char *run_metal_motion_v2(double *out_score)
{
    *out_score = NAN;
    int err = 0;
    ...
    err = vmaf_feature_score_at_index(vmaf, "VMAF_integer_feature_motion2_v2_score", out_score, 0u);
    mu_assert("Metal: vmaf_feature_score_at_index(motion2_v2, idx=0) failed", !err);
...
static char *test_motion_v2_cpu_metal_parity(void)
{
    ...
    if (msg)
        return msg;
    msg = run_metal_motion_v2(&metal_score);
    if (msg)
        return msg;
    ...
    if (delta > PARITY_TOL) {
        (void)fprintf(stderr, "\nmotion2_v2 parity FAIL: cpu=%.8f metal=%.8f delta=%.2e tol=%.2e\n",
                      cpu_score, metal_score, delta, PARITY_TOL);
    }
    mu_assert("motion2_v2 CPU vs. Metal delta exceeds places=4 tolerance (1e-4)",
              delta <= PARITY_TOL);
    return NULL;
}
