...
/* Look up the "float_ms_ssim" extractor, create+init a context over an
 * FMSS_W x FMSS_H 8-bit YUV420P frame, then init a feature collector.
 * Both tests below start this way; neither uses dictionary options. */
static char *float_ms_ssim_fixture_open(VmafFeatureExtractorContext **ctx,
...
static char *test_float_ms_ssim_8bit_identical(void)
{
    ...
    char *msg = float_ms_ssim_fixture_open(&ctx, &fc);
    if (msg)
        return msg;
