...
#include "config.h"
#include "test.h"
/* ADR-0729 Wave 3: model.c renamed to model.cpp; include the .cpp unity source. */
#include "model.cpp"
...
static char *slurp(const char *path, size_t *len)
{
    ...
    char *buf = static_cast<char *>(calloc(file_sz + 1u, 1u));
    if (!buf) {
        (void)fclose(f);
        return NULL;
    }
...
static char *test_version_next(void)
{
    ...
    while ((next = vmaf_model_version_next(next, &version)) != NULL) {
        const VmafBuiltInModel *m = static_cast<const VmafBuiltInModel *>(next);
