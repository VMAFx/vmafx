 ...
 * CLI round-trip against the Netflix golden pair under
 * `--cpumask ~0x8` (scalar) vs default (AVX2). See the deep-dive
 * digest accompanying this PR. An inline-scalar-ref test cannot
 * reproduce production bit-exactness because the test TU and the
 * scalar-extractor TU compile under different cflag sets (the test
 * lacks `-mavx -mavx2 -mfma` by design — those are AVX-lib scope),
