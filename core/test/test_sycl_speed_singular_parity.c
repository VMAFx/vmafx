...
/*
 * ADR-1218 — SpEED singular-covariance parity (SYCL).
 *
 ...
 *
 * `speed_temporal_sycl`'s run_cpu_linalg never reported singularity at all, so
 ...
 *
 * Skip behaviour: if vmaf_sycl_state_init() fails (no oneAPI runtime or no device
 * visible) every test emits "[skip: no SYCL device]" and passes.
 */
...
{
    return assert_parity("speed_temporal", "speed_temporal_sycl", MODE_TEMPORAL_ONE_SIDED,
...
{
    return assert_parity("speed_temporal", "speed_temporal_sycl", MODE_TEMPORAL_BOTH_SINGULAR,
                         "Speed_temporal_feature_speed_temporal_score", TEMPORAL_FRAMES - 1u,
...
{
    return assert_parity("speed_chroma", "speed_chroma_sycl", MODE_CHROMA_BOTH_SINGULAR,
