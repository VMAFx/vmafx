...
/*
 * ADR-1218 — SpEED singular-covariance parity (CUDA).
 *
 ...
 *
 * `speed_temporal_cuda`'s run_cpu_linalg never reported singularity at all, so
...
{
    return assert_parity("speed_temporal", "speed_temporal_cuda", MODE_TEMPORAL_ONE_SIDED,
...
{
    return assert_parity("speed_temporal", "speed_temporal_cuda", MODE_TEMPORAL_BOTH_SINGULAR,
                         "Speed_temporal_feature_speed_temporal_score", TEMPORAL_FRAMES - 1u,
...
{
    return assert_parity("speed_chroma", "speed_chroma_cuda", MODE_CHROMA_BOTH_SINGULAR,
