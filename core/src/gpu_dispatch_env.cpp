...
static constexpr unsigned kEnvTableCap = 8U;

/* One row per distinct var_name. */
struct EnvRow {
    const char *var_name; /* points to the caller's string literal; nullptr = free */
    const char *value;    /* strdup copy; nullptr if var was unset at snapshot time */
};

static EnvRow g_rows[kEnvTableCap];
...
{
    (void)InitOnceExecuteOnce(&g_lock_once, init_lock_cb, nullptr, nullptr);
...
const char *vmaf_gpu_dispatch_env_get(const char *var_name)
{
    ...
    for (unsigned i = 0U; i < kEnvTableCap; i++) {
    ...
    for (unsigned i = 0U; i < kEnvTableCap; i++) {
        if (g_rows[i].var_name && std::strcmp(g_rows[i].var_name, var_name) == 0)
            return g_rows[i].value;
    }
    ...
    const char *val = std::getenv(var_name); /* NOLINT(concurrency-mt-unsafe) */
