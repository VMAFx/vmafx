...
void pool_destruct_partial(VmafPicturePool *p, unsigned stage)
{
...
int pool_construct(VmafPicturePool **pool, const VmafPicturePoolConfig &cfg, unsigned *stage)
{
...
void pool_return_index(VmafPicturePool *pool, unsigned idx)
{
...
int pool_pop_slot(VmafPicturePool *pool, unsigned *idx, VmafPicture *snapshot)
{
 ...
 * exactly what it had allocated and leaves pic->priv nullptr, matching the three
 * former `goto return_to_pool` sites; pushing the index back is the caller's
 * job, so the free-list handling stays in one place. */
int pool_attach_priv(VmafPicturePool *pool, VmafPicture *pic, unsigned idx)
{
    ...
    int err = vmaf_picture_set_release_callback(pic, nullptr, pooled_picture_release);
    if (err) {
        std::free(priv);
