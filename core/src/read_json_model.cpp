...
void sync_n_features(VmafModel *model, unsigned count)
{
...
int parse_libsvm_model(json_stream *s, VmafModel *model)
{
     ...
     * A duplicate `model` key re-enters this function, and an unconditional
     * assignment orphaned the first svm_model: nothing else holds a pointer
     * to it, so neither vmaf_model_destroy nor svm_free_and_destroy_model can
     * reach it and the struct plus its sv_coef rows leak. Found by the nightly
     * fuzz_json_model LeakSanitizer lane (184 B direct + 64 B indirect).
     *
     * Same shape, and the same fix, as the duplicate `feature_names` key
     * handled in append_feature_name (ADR-0887). Keep this in sync with the
     * C twin core/src/read_json_model.c, which the fuzz harness compiles
     * directly (core/test/fuzz/meson.build) while the library builds this
...
int validate_feature_arrays(VmafModel *model)
{
...
int parse_model_dict(json_stream *s, VmafModel *model, enum VmafModelFlags flags)
{
    ...
    const int err = validate_feature_arrays(model);
    if (err)
        return err;
...
[[nodiscard]] int vmaf_read_json_model_from_path(VmafModel **model, VmafModelConfig *cfg,
                                                 const char *path)
{
    int err = 0;
                                ...
                                const char *name, char *cfg_name, size_t cfg_name_sz)
{
    ...
    while (json_peek(s) != JSON_OBJECT_END && !json_get_error(s)) {
        ...
        if (err) {
             ...
             * *model_collection is still nullptr, so leave them untouched
                           ...
                           VmafModelCollection **model_collection, VmafModelConfig *cfg)
{
    *model_collection = nullptr;
    ...
    char *cfg_name = static_cast<char *>(malloc(cfg_name_sz));
    if (!cfg_name) {
...
[[nodiscard]] int vmaf_read_json_model_collection_from_buffer(
    VmafModel **model, VmafModelCollection **model_collection, VmafModelConfig *cfg,
    const char *data, const int data_len)
{
    int err = 0;
    json_stream s;
