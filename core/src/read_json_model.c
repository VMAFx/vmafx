 ...
 * mismatch is later read by `vmaf_model_destroy()` and (per
 * ADR-0887) any downstream consumer that walks `[0, n_features)`;
 * leaving `n_features` stale lets the destroy walk one bound while
 * the rest of the API sees a smaller count, hiding bad models from
 * the final cross-key validation in `parse_model_dict()`. */
...
static int parse_feature_names(json_stream *s, VmafModel *model)
{
    ...
    while (json_peek(s) != JSON_ARRAY_END && !json_get_error(s)) {
        ...
        if (err)
            return err;
        /* Per-iteration sync (rather than once at loop end) so an early
         * -EINVAL on a later element still leaves `n_features` covering the
         * slots we already strdup'd — the parser's fail-path
         * `vmaf_model_destroy` needs to see those names to free them.
         * Switched from the prior unconditional `n_features++` (which
         * double-counted on repeated `feature_names` keys in fuzzer-mangled
         * JSON) to `sync_n_features` max-merge per ADR-0887. */
        sync_n_features(model, i);
    }
 ...
 * `parse_feature_names`. A missing name means the JSON's per-feature
 * arrays disagree on length — a malformed model. Reject at parse time
 * (per ADR-0887) so downstream consumers don't have to defend against
