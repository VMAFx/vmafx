...
int vmaf_use_tiny_model(VmafContext *ctx, const char *onnx_path, const VmafDnnConfig *cfg)
{
    ...
    size_t in_rank = 0;
    /* load_path, not onnx_path: the int8 redirect above may have pointed this
     * at the `.int8.onnx` sidecar, and opening onnx_path here would compute the
     * redirect and then silently ignore it. They are the same pointer whenever
     * no redirect applied. */
    rc = open_session_and_probe_input(load_path, cfg, &sess, in_shape, &in_rank);
    if (rc < 0) {
        if (have_meta)
            vmaf_dnn_sidecar_free(&meta);
        return rc;
