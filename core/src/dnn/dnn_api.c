...
int vmaf_dnn_session_open(VmafDnnSession **out, const char *onnx_path, const VmafDnnConfig *cfg)
{
    ...
    /* Both fallback triggers of the int8 redirect: resolve_load_path() covers
     * an int8 file that fails the size cap or op allowlist, and the retry
     * inside vmaf_ort_open_with_fallback() covers one the runtime cannot open. */
    if (rc == 0)
        rc = vmaf_ort_open_with_fallback(&s->ort, load_path, onnx_path, cfg);
    if (rc < 0) {
        if (s->has_sidecar)
