...
int vmaf_dnn_session_open(VmafDnnSession **out, const char *onnx_path, const VmafDnnConfig *cfg)
{
    ...
    if (rc == 0) {
        ...
        if (rc < 0 && load_path != onnx_path) {
             ...
             * "better degraded than dead" rule as the missing-file branch in
             * resolve_load_path(): the redirect must never turn a call that
             * worked against the fp32 baseline into a hard failure.
             * vmaf_ort_open() leaves `s->ort` untouched on error, so the retry
             * cannot leak the failed session. Kept identical to
