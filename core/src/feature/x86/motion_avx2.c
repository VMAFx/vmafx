...
void x_convolution_16_avx2(const uint16_t *src, uint16_t *dst, unsigned width, unsigned height,
                           ptrdiff_t src_stride, ptrdiff_t dst_stride)
{
    ...
    for (unsigned i = 0; i < height; ++i) {
        ...
        for (unsigned j = 0; j < vector_loop; j = j + 1) {
            ...
            _mm256_storeu_si256((__m256i *)(dst + i * dst_stride + j * 16 + left_edge), result);
    ...
    *sad = 0;
    ...
    for (unsigned i = 0; i < height; i++) {
        ...
        *sad += inner_sad;
        ...
        final_accum = _mm256_add_epi64(
            final_accum,
            _mm256_cvtepi32_epi64(_mm_add_epi32(_mm256_castsi256_si128(inter_accum_lo),
                                                _mm256_extracti128_si256(inter_accum_lo, 1))));
        final_accum = _mm256_add_epi64(
            final_accum,
            _mm256_cvtepi32_epi64(_mm_add_epi32(_mm256_castsi256_si128(inter_accum_hi),
                                                _mm256_extracti128_si256(inter_accum_hi, 1))));
