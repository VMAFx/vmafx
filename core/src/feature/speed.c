...
static float compute_householder_transform(float *A, int col, int start_row, int size)
{
    if (size - start_row == 1)
        return 0.0f;
...
static void convert_to_tridiagonal(float *A, int size, float *d, float *sd, float *buffer)
{
    float *v = buffer;
    buffer += size;
    float *x = buffer;
...
static void compute_eigenvalues_tridiagonal(float *d, float *sd, float *eigenvalues, int size)
{
    ...
    if (iter == EIGENVALUE_MAX_ITERS) {
        vmaf_log(VMAF_LOG_LEVEL_WARNING, "compute_eigenvalues_tridiagonal: max iterations reached, "
    ...
    for (int i = R->rows - 1; i >= 0; i--) {
        ...
        for (int j = 0; j < X->cols; j++) {
            ...
            for (int k = i + 1; k < R->rows; k++) {
                independent_term -= (X->data[k * X->cols + j] * R->data[i * R->cols + k]);
            }
                               ...
                               float *output_data, float *tmp_buffer)
{
    ...
    Matrix A;
    matrix_init(&A, A_size, A_size, A_buffer);
    matrix_copy(&A, &A_immutable);

    Matrix Q;
    matrix_init(&Q, A_size, A_size, Q_buffer);
    Matrix R;
    matrix_init(&R, A_size, A_size, R_buffer);
    Matrix tmp1;
    matrix_init(&tmp1, A_size, A_size, tmp1_buffer);
    Matrix tmp2;
    matrix_init(&tmp2, A_size, A_size, tmp2_buffer);

    Matrix B_immutable;
    matrix_init(&B_immutable, A_size, B_cols, B_data);

    Matrix tmp_rect;
    matrix_init(&tmp_rect, A_size, B_cols, tmp_rect_buffer);
    Matrix X;
    matrix_init(&X, A_size, B_cols, output_data);
                                ...
                                int start_col_y)
{
    ...
    for (size_t i = 0; i < dim.submatrix_height; i++) {
        for (size_t j = 0; j < dim.submatrix_width; j++) {
            double val_x = data[(start_row_x + i) * stride_px + (start_col_x + j)];
            double val_y = data[(start_row_y + i) * stride_px + (start_col_y + j)];
...
static int est_params(SpeedState *s, const float *data, float sigma_nn, SpeedResultBuffers *output)
{
    ...
    // Step 10: Return S, E
    memcpy(output->variances, s->buffers.linear_system_sol, dim.num_blocks * sizeof(float));
...
static int speed_init_dimensions(SpeedDimensions *dim, int w, int h, double speed_prescale)
{
...
int speed_init(SpeedState *s, SpeedOptions *opt, int w, int h)
{
    ...
    if (!s->buffers.cov_mat)
        return -ENOMEM;
    s->buffers.eigenvalues = aligned_malloc(sizeof(float) * dim->elements_in_block, 32);
                       ...
                       unsigned w, unsigned h)
{
    ...
    s->frame_buffer_ref = aligned_malloc(s->speed_state.float_stride * dim.alloc_height, 32);
    if (!s->frame_buffer_ref)
        return -ENOMEM;
    s->frame_buffer_dis = aligned_malloc(s->speed_state.float_stride * dim.alloc_height, 32);
                             ...
                             int channel, float *score)
{
    picture_copy(s->frame_buffer_ref, s->speed_state.float_stride, ref_pic, -128, ref_pic->bpc,
                 channel);
    picture_copy(s->frame_buffer_dis, s->speed_state.float_stride, dist_pic, -128, dist_pic->bpc,
                 channel);
    return speed_extract_score(&s->speed_state, &s->speed_options, s->frame_buffer_ref,
                ...
                unsigned h)
{
    ...
    if (!s->frame_buffer_ref[0] || !s->frame_buffer_ref[1] || !s->frame_buffer_dis[0] ||
        !s->frame_buffer_dis[1]) {
        return -ENOMEM;
    }
                   ...
                   VmafFeatureCollector *feature_collector)
{
    ...
    picture_copy(s->frame_buffer_ref[cyclic_index], s->speed_state.float_stride, ref_pic, -128,
                 ref_pic->bpc, 0);
    picture_copy(s->frame_buffer_dis[cyclic_index], s->speed_state.float_stride, dist_pic, -128,
                 ref_pic->bpc, 0);
    ...
    if (s->speed_temporal_use_ref_diff) {
        subtract_image(s->frame_buffer_dis[other_index], s->frame_buffer_ref[cyclic_index], w, h,
                       float_stride);
    } else {
        subtract_image(s->frame_buffer_dis[other_index], s->frame_buffer_dis[cyclic_index], w, h,
                       float_stride);
    }
    float score;
...
static int close(VmafFeatureExtractor *fex)
{
    ...
    if (s->frame_buffer_ref[0])
        aligned_free(s->frame_buffer_ref[0]);
    if (s->frame_buffer_ref[1])
        aligned_free(s->frame_buffer_ref[1]);
    if (s->frame_buffer_dis[0])
        aligned_free(s->frame_buffer_dis[0]);
    if (s->frame_buffer_dis[1])
        aligned_free(s->frame_buffer_dis[1]);
