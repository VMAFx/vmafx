...
{
    int src_px_stride = src_stride / (int)sizeof(float);
    int dst_px_stride = dst_stride / (int)sizeof(float);
    for (int i = 0; i < h; ++i) {
        for (int j = 0; j < w; ++j) {
                                     ...
                                     int top, int right, int bottom, float factor)
{
    (void)w;
    (void)h;
    int px_stride = src_stride / (int)sizeof(float);
    double accum = 0.0;
                                 ...
                                 int right, int bottom)
{
    (void)w;
    (void)h;
    int px_stride = stride / (int)sizeof(float);
    double accum = 0.0;
    for (int i = top; i < bottom; ++i) {
        double row = 0.0;
        for (int j = left; j < right; ++j) {
...
/* Public accessor so float_adm.c can prime the dispatch table during init(). */
               ...
               double adm_f2s3)
{
     ...
     * Pass region-aware sub-band pointers offset to [top,left] so the SIMD
     * function processes [0,band_h) x [0,band_w) internally. */
    adm_init_simd_dispatch();
    ...
    for (theta = 0; theta < 3; ++theta) {
        const float *sp = src_angles[theta] + top * src_px_stride + left;
        float *dp = dst_angles[theta] + top * dst_px_stride + left;
        float *fp = flt_angles[theta] + top * dst_px_stride + left;
