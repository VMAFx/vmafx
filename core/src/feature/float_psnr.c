                ...
                unsigned h)
{
    ...
    if (bpc == 8) {
        ...
        /* Unsupported bitdepth. Upstream parity: the buffers allocated above
         * are intentionally left to the extractor teardown path, exactly as
         * the former `fail:` unwind target did (it skipped `free_ref:`). */
        return -ENOMEM;
    }
