                          ...
                          bool singular_v)
{
    ...
    int err = speed_internal_clamp_score(score_u, mxv, index, "speed_chroma_hip", "speed_chroma_u",
                                         &clamped_u);
    if (err)
        return err;
    err = speed_internal_clamp_score(score_v, mxv, index, "speed_chroma_hip", "speed_chroma_v",
                                     &clamped_v);
    if (err)
        return err;
    err = speed_internal_clamp_score(score_uv, mxv, index, "speed_chroma_hip", "speed_chroma_uv",
                                     &clamped_uv);
    if (err)
        return err;
