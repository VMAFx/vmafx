...
`ind_size_y` and eight further device-pointer fields. The four
`__global__` kernels that read it —

- `adm_csf_kernel_1_4` and `i4_adm_csf_kernel_1_4`
  (`integer_adm/adm_csf.hip`, generated by the two macros),
- `adm_cm_line_kernel_8` and `i4_adm_cm_line_kernel`
  (`integer_adm/adm_cm.hip`)

— take `const AdmBufferHip *__restrict__ buf_ptr`, never the struct by
value. `adm_cm_reduce_line_kernel_4` does not read the struct at all
and takes neither form.

**History, because this has been reverted once already.** ADR-0759
landed the pointer convention in `31a51afb2` (PR #101). The next merge,
`92ea978a4` (PR #102, an unrelated CUDA ciede change cut from an older
base), silently restored the by-value signatures; the AGENTS.md note
survived, so for months this file asserted an invariant the kernels did
not hold. If a rebase or a squash-merge of a stale branch reintroduces
...
1. `init_fex_hip()` allocates it with `hipMalloc` and uploads it with
   `hipMemcpy(hipMemcpyHostToDevice)` **after** the band-slicing and
   result-slicing blocks, once every pointer inside `s->buf` is final.
2. Each of the four launch helpers passes `&s->buf_dev` — the address
   ...
   address — as `args[0]`.
3. `close_fex_hip()` frees it; so does the `fail_buf_dev` label on the
   init failure ladder.
...
`init_fex_hip()` and cleared in `close_fex_hip()`, and `submit()` hands
`&s->buf` to the launch helpers without modifying it. Any future code
that changes a field of `s->buf` after init must re-upload the copy
before the next launch, or the kernels will read stale pointers.

**Measured** (ROCm 7.2 `hipcc --genco`, per-kernel metadata; same delta
on `gfx1036`, `gfx1100` and `gfx90a`):

| kernel | kernarg by value | by pointer |
| :--- | ---: | ---: |
| `adm_csf_kernel_1_4` | 856 | 536 |
| `i4_adm_csf_kernel_1_4` | 856 | 536 |
| `i4_adm_cm_line_kernel` | 904 | 584 |
| `adm_cm_line_kernel_8` | 968 | 648 |

320 bytes = `sizeof(AdmBufferHip) - sizeof(void *)`, off every launch.
`adm_cm_line_kernel_8`'s per-thread scratch also drops with it (920 →
608 bytes on `gfx1036`, 932 → 616 on `gfx90a`, 664 → 352 on `gfx1100`):
the by-value copy was being spilled.

`AdmFixedParametersHip` (248 bytes) is still passed **by value** to all
