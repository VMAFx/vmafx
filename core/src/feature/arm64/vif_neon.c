...
#define NEON_FILTER_INSTANCE_U32X4_MULL_U32X4_WITH_CONST_LU4(vec_accum_name, vec_init, vec_name,   \
                                                             const_val)                            \
...
#define NEON_FILTER_UPDATE_U32X4_ACCUM_MULL_U32X4_WITH_CONST_LU4(vec_accum_name, vec_name,         \
                                                                 const_val)                        \
...
#define NEON_FILTER_OFFSET_SHIFT_STORE_U32X4_HI_LO_LU2(accum_name, offset_vec, shift_vec,          \
                                                       store_pointer)                              \
    NEON_FILTER_OFFSET_SHIFT_STORE_U32X4(accum_name##_0_l, offset_vec, shift_vec, store_pointer)   \
    NEON_FILTER_OFFSET_SHIFT_STORE_U32X4(accum_name##_0_h, offset_vec, shift_vec,                  \
                                         (store_pointer + 4))                                      \
    NEON_FILTER_OFFSET_SHIFT_STORE_U32X4(accum_name##_1_l, offset_vec, shift_vec,                  \
                                         (store_pointer + 8))                                      \
    NEON_FILTER_OFFSET_SHIFT_STORE_U32X4(accum_name##_1_h, offset_vec, shift_vec,                  \
                                         (store_pointer + 12))
...
#define NEON_FILTER_INSTANCE_U32X4_NO_INIT_MULL_U16X4_WITH_CONST_LO_HI(accum_name, vec_name,       \
                                                                       const_val)                  \
...
#define NEON_FILTER_INSTANCE_U32X4_NO_INIT_MULL_U16X4_WITH_CONST_LO_HI_LU2(accum_name, vec_name,   \
                                                                           const_val)              \
    NEON_FILTER_INSTANCE_U32X4_NO_INIT_MULL_U16X4_WITH_CONST_LO_HI(accum_name##_0, vec_name##_0,   \
                                                                   const_val)                      \
    NEON_FILTER_INSTANCE_U32X4_NO_INIT_MULL_U16X4_WITH_CONST_LO_HI(accum_name##_1, vec_name##_1,   \
                                                                   const_val)

// The macro instance and initialize 2 accumulators (low and high) with the multiplication of a uint16x8_t vector by a scalar (filter element)
#define NEON_FILTER_INSTANCE_U32X4_INIT_MULL_U16X4_WITH_CONST_LO_HI(accum_name, vec_init,          \
                                                                    vec_name, const_val)           \
...
// Using the macro above with loop unrool (LU2)
#define NEON_FILTER_INSTANCE_U32X4_INIT_MULL_U16X4_WITH_CONST_LO_HI_LU2(accum_name, vec_init,      \
                                                                        vec_name, const_val)       \
    NEON_FILTER_INSTANCE_U32X4_INIT_MULL_U16X4_WITH_CONST_LO_HI(accum_name##_0, vec_init,          \
                                                                vec_name##_0, const_val)           \
    NEON_FILTER_INSTANCE_U32X4_INIT_MULL_U16X4_WITH_CONST_LO_HI(accum_name##_1, vec_init,          \
                                                                vec_name##_1, const_val)

// Using the macro above with low and high vectors (LH)
#define NEON_FILTER_INSTANCE_U32X4_INIT_MULL_U16X4_WITH_CONST_LO_HI_LH(accum_name, vec_init,       \
                                                                       vec_name, const_val)        \
    NEON_FILTER_INSTANCE_U32X4_INIT_MULL_U16X4_WITH_CONST_LO_HI(accum_name##_l, vec_init,          \
                                                                vec_name##_l, const_val)           \
    NEON_FILTER_INSTANCE_U32X4_INIT_MULL_U16X4_WITH_CONST_LO_HI(accum_name##_h, vec_init,          \
                                                                vec_name##_h, const_val)
...
#define NEON_FILTER_SHIFT_UNZIP_STORE_U32X4_TO_U16X8_LU2(vec_name, shift_vec, store_address)       \
    {                                                                                              \
        uint16x8_t vec_name##_tmp_0 =                                                              \
            vuzp1q_u16(vreinterpretq_u16_u32(vshlq_u32(vec_name##_0, shift_vec)),                  \
                       vreinterpretq_u16_u32(vshlq_u32(vec_name##_1, shift_vec)));                 \
        uint16x8_t vec_name##_tmp_1 =                                                              \
            vuzp1q_u16(vreinterpretq_u16_u32(vshlq_u32(vec_name##_2, shift_vec)),                  \
...
#define NEON_FILTER_INSTANCE_U64X2_INIT_MULL_U32X2_LO_HI(accum_name, init_vec, vec_name_a,         \
                                                         vec_name_b)                               \
    uint64x2_t accum_name##_l =                                                                    \
        vmlal_u32(init_vec, vget_low_u32(vec_name_a), vget_low_u32(vec_name_b));                   \
...
#define NEON_FILTER_INSTANCE_U64X2_INIT_MULL_U32X2_WITH_CONST_LO_HI(accum_name, init_vec,          \
                                                                    vec_name, const_val)           \
...
// Using the macro above with loop unrool (LU2)
#define NEON_FILTER_INSTANCE_U64X2_INIT_MULL_U32X2_WITH_CONST_LO_HI_LU2(accum_name, init_vec,      \
                                                                        vec_name, const_val)       \
    NEON_FILTER_INSTANCE_U64X2_INIT_MULL_U32X2_WITH_CONST_LO_HI(accum_name##_0, init_vec,          \
                                                                vec_name##_0, const_val)           \
    NEON_FILTER_INSTANCE_U64X2_INIT_MULL_U32X2_WITH_CONST_LO_HI(accum_name##_1, init_vec,          \
                                                                vec_name##_1, const_val)

// Using the macro above with low and high vectors (LH)
#define NEON_FILTER_INSTANCE_U64X2_INIT_MULL_U32X2_WITH_CONST_LO_HI_LH(accum_name, init_vec,       \
                                                                       vec_name, const_val)        \
    NEON_FILTER_INSTANCE_U64X2_INIT_MULL_U32X2_WITH_CONST_LO_HI(accum_name##_l, init_vec,          \
                                                                vec_name##_l, const_val)           \
    NEON_FILTER_INSTANCE_U64X2_INIT_MULL_U32X2_WITH_CONST_LO_HI(accum_name##_h, init_vec,          \
                                                                vec_name##_h, const_val)
...
#define NEON_FILTER_UPDATE_U64X2_ACCUM_LO_HI(accum_name, vec_name_a, vec_name_b)                   \
    accum_name##_l =                                                                               \
        vmlal_u32(accum_name##_l, vget_low_u32(vec_name_a), vget_low_u32(vec_name_b));             \
...
#define NEON_FILTER_INSTANCE_U64X2_INIT_MULL_SHIFT_UNZIP_U32X4_LO_HI(                              \
    out_vec_name, init_vec, vec_name_in_a, vec_name_in_b, shift_vec)                               \
    uint64x2_t out_vec_name##_l =                                                                  \
        vmlal_u32(init_vec, vget_low_u32(vec_name_in_a), vget_low_u32(vec_name_in_b));             \
    uint64x2_t out_vec_name##_h = vmlal_high_u32(init_vec, vec_name_in_a, vec_name_in_b);          \
    uint32x4_t out_vec_name =                                                                      \
        vuzp1q_u32(vreinterpretq_u32_u64(vshlq_u64(out_vec_name##_l, shift_vec)),                  \
...
#define NEON_FILTER_SHIFT_UNZIP_STORE_U64X2_TO_U32X4_LO_HI(accum_name, shift_vec, store_pointer)   \
    uint32x4_t accum_name##_tmp =                                                                  \
        vuzp1q_u32(vreinterpretq_u32_u64(vshlq_u64(accum_name##_l, shift_vec)),                    \
                   vreinterpretq_u32_u64(vshlq_u64(accum_name##_h, shift_vec)));                   \
...
#define NEON_FILTER_LOAD_U8X8_MOVE_TO_U16X8_AND_SQR_LU2(vec_name, vec16_name, sqr_vec_name,        \
                                                        load_address)                              \
...
#define NEON_FILTER_LOAD_U16X8_AND_MOVE_TO_U32X4_AND_SQR(vec_name, vec32_name, sqr_vec_name,       \
                                                         load_address)                             \
    ...
    for (unsigned int i = 0; i < h; ++i, i_dst_stride += dst_stride) {
        ...
        for (; j < uiw15; j += 16, p_ref += 16, p_dis += 16) {
            ...
            NEON_FILTER_INSTANCE_U32X4_INIT_MULL_U16X4_WITH_CONST_LO_HI_LU2(
                accum_f_ref, offset_vec_v, ref_vec_16u, vif_filt_s1[0]);
            NEON_FILTER_INSTANCE_U32X4_INIT_MULL_U16X4_WITH_CONST_LO_HI_LU2(
            ...
            for (unsigned fi = 1; fi < fwidth; ++fi, pp_ref += buf->stride, pp_dis += buf->stride) {
                ...
                NEON_FILTER_UPDATE_ACCUM_U32X4_WITH_CONST_LO_HI_LU2(accum_f_ref, ref_vec_16u,
                                                                    vif_filt_s1[fi]);
                NEON_FILTER_UPDATE_ACCUM_U32X4_WITH_CONST_LO_HI_LU2(accum_f_dis, dis_vec_16u,
                                                                    vif_filt_s1[fi]);
            }
        ...
        for (; j < uiw15; j += 16, pRefConv += 16, pDisConv += 16) {
            ...
            NEON_FILTER_INSTANCE_U32X4_MULL_U32X4_WITH_CONST_LU4(accum_ref_conv, offset_vec_h,
                                                                 ref_conv_vec_u32, vif_filt_s1[0]);
            NEON_FILTER_INSTANCE_U32X4_MULL_U32X4_WITH_CONST_LU4(accum_dis_conv, offset_vec_h,
                                                                 dis_conv_vec_u32, vif_filt_s1[0]);
            ...
            for (unsigned fj = 1; fj < fwidth; ++fj) {
                ...
                NEON_FILTER_UPDATE_U32X4_ACCUM_MULL_U32X4_WITH_CONST_LU4(
                    accum_ref_conv, ref_conv_vec_u32, vif_filt_s1[fj]);
                NEON_FILTER_UPDATE_U32X4_ACCUM_MULL_U32X4_WITH_CONST_LU4(
                    accum_dis_conv, dis_conv_vec_u32, vif_filt_s1[fj]);
            }
                              ...
                              int bpc)
{
    ...
    for (unsigned i = 0; i < h; ++i, i_dst_stride += stride_h) {
        ...
        for (; j < uiw15; j += 16, p_ref += 16, p_dis += 16) {
            ...
            NEON_FILTER_INSTANCE_U32X4_INIT_MULL_U16X4_WITH_CONST_LO_HI_LH(
                accum_f_ref, add_shift_round_VP_vec, ref_vec_16u, vif_filt_s[0]);
            NEON_FILTER_INSTANCE_U32X4_INIT_MULL_U16X4_WITH_CONST_LO_HI_LH(
                accum_f_dis, add_shift_round_VP_vec, dis_vec_16u, vif_filt_s[0]);
            ...
            for (unsigned fi = 1; fi < fwidth; ++fi, pp_ref += stride_v, pp_dis += stride_v) {
                ...
                NEON_FILTER_UPDATE_ACCUM_U32X4_WITH_CONST_LO_HI_LH(accum_f_ref, ref_vec_16u,
                                                                   vif_filt_s[fi]);
                NEON_FILTER_UPDATE_ACCUM_U32X4_WITH_CONST_LO_HI_LH(accum_f_dis, dis_vec_16u,
                                                                   vif_filt_s[fi]);
            }
        ...
        for (; j < uiw15; j += 16, pRefConv += 16, pDisConv += 16) {
            ...
            NEON_FILTER_INSTANCE_U32X4_MULL_U32X4_WITH_CONST_LU4(accum_ref_conv, offset_vec_h,
                                                                 ref_conv_vec_u32, vif_filt_s[0]);
            NEON_FILTER_INSTANCE_U32X4_MULL_U32X4_WITH_CONST_LU4(accum_dis_conv, offset_vec_h,
                                                                 dis_conv_vec_u32, vif_filt_s[0]);
            ...
            for (unsigned fj = 1; fj < fwidth; ++fj) {
                ...
                NEON_FILTER_UPDATE_U32X4_ACCUM_MULL_U32X4_WITH_CONST_LU4(
                    accum_ref_conv, ref_conv_vec_u32, vif_filt_s[fj]);
                NEON_FILTER_UPDATE_U32X4_ACCUM_MULL_U32X4_WITH_CONST_LU4(
                    accum_dis_conv, dis_conv_vec_u32, vif_filt_s[fj]);
            }
...
void vif_statistic_8_neon(struct VifPublicState *s, float *num, float *den, unsigned w, unsigned h)
{
    ...
    for (unsigned i = 0; i < h; ++i, i_dst_stride += dst_stride) {
        ...
        unsigned int j = 0;
        for (; j < uiw15; j += 16, p_ref += 16, p_dis += 16) {
            NEON_FILTER_LOAD_U8X8_MOVE_TO_U16X8_AND_SQR_LU2(ref_vec_8u, ref_vec_16u,
                                                            ref_ref_vec_16u, p_ref);
            NEON_FILTER_LOAD_U8X8_MOVE_TO_U16X8_AND_SQR_LU2(dis_vec_8u, dis_vec_16u,
                                                            dis_dis_vec_16u, p_dis);

            NEON_FILTER_INSTANCE_U32X4_NO_INIT_MULL_U16X4_WITH_CONST_LO_HI_LU2(
                accum_f_ref, ref_vec_16u, vif_filt_s0[0])
            NEON_FILTER_INSTANCE_U32X4_NO_INIT_MULL_U16X4_WITH_CONST_LO_HI_LU2(
                accum_f_dis, dis_vec_16u, vif_filt_s0[0]);
            NEON_FILTER_INSTANCE_U32X4_NO_INIT_MULL_U16X4_WITH_CONST_LO_HI_LU2(
                accum_f_ref_ref, ref_ref_vec_16u, vif_filt_s0[0]);
            NEON_FILTER_INSTANCE_U32X4_NO_INIT_MULL_U16X4_WITH_CONST_LO_HI_LU2(
                accum_f_dis_dis, dis_dis_vec_16u, vif_filt_s0[0]);

            uint32x4_t accum_f_ref_dis_0_l =
                vmulq_u32(accum_f_dis_0_l, vmovl_u16(vget_low_u16(ref_vec_16u_0)));
            uint32x4_t accum_f_ref_dis_0_h =
                vmulq_u32(accum_f_dis_0_h, vmovl_high_u16(ref_vec_16u_0));

            uint32x4_t accum_f_ref_dis_1_l =
                vmulq_u32(accum_f_dis_1_l, vmovl_u16(vget_low_u16(ref_vec_16u_1)));
            uint32x4_t accum_f_ref_dis_1_h =
                vmulq_u32(accum_f_dis_1_h, vmovl_high_u16(ref_vec_16u_1));

            const uint8_t *pp_ref = p_ref + buf.stride;
            const uint8_t *pp_dis = p_dis + buf.stride;
            for (unsigned int fi = 1; fi < fwidth;
                 ++fi, pp_ref += buf.stride, pp_dis += buf.stride) {
                NEON_FILTER_LOAD_U8X8_MOVE_TO_U16X8_AND_SQR_LU2(ref_vec_8u, ref_vec_16u,
                                                                ref_ref_vec_16u, pp_ref);
                NEON_FILTER_LOAD_U8X8_MOVE_TO_U16X8_AND_SQR_LU2(dis_vec_8u, dis_vec_16u,
                                                                dis_dis_vec_16u, pp_dis);

                NEON_FILTER_INSTANCE_U32X4_NO_INIT_MULL_U16X4_WITH_CONST_LO_HI_LU2(
                    f_dis, dis_vec_16u, vif_filt_s0[fi]);

                NEON_FILTER_UPDATE_ACCUM_U32X4_WITH_CONST_LO_HI_LU2(accum_f_ref, ref_vec_16u,
                                                                    vif_filt_s0[fi]);
                NEON_FILTER_UPDATE_ACCUM_U32X4_WITH_CONST_LO_HI_LU2(accum_f_dis, dis_vec_16u,
                                                                    vif_filt_s0[fi]);
                NEON_FILTER_UPDATE_ACCUM_U32X4_WITH_CONST_LO_HI_LU2(
                    accum_f_ref_ref, ref_ref_vec_16u, vif_filt_s0[fi]);
                NEON_FILTER_UPDATE_ACCUM_U32X4_WITH_CONST_LO_HI_LU2(
                    accum_f_dis_dis, dis_dis_vec_16u, vif_filt_s0[fi]);

                accum_f_ref_dis_0_l = vmlaq_u32(accum_f_ref_dis_0_l, f_dis_0_l,
                                                vmovl_u16(vget_low_u16(ref_vec_16u_0)));
                accum_f_ref_dis_0_h =
                    vmlaq_u32(accum_f_ref_dis_0_h, f_dis_0_h, vmovl_high_u16(ref_vec_16u_0));
                accum_f_ref_dis_1_l = vmlaq_u32(accum_f_ref_dis_1_l, f_dis_1_l,
                                                vmovl_u16(vget_low_u16(ref_vec_16u_1)));
                accum_f_ref_dis_1_h =
                    vmlaq_u32(accum_f_ref_dis_1_h, f_dis_1_h, vmovl_high_u16(ref_vec_16u_1));
            }
            NEON_FILTER_OFFSET_SHIFT_STORE_U32X4_HI_LO_LU2(accum_f_ref, offset_vec_v, shift_vec_v,
                                                           buf.tmp.mu1 + j);
            NEON_FILTER_OFFSET_SHIFT_STORE_U32X4_HI_LO_LU2(accum_f_dis, offset_vec_v, shift_vec_v,
                                                           buf.tmp.mu2 + j);
        ...
        for (; j < uiw7; j += 8, pMul1 += 8, pMul2 += 8, pDis += 8, pRef += 8, pRefDis += 8) {
            ...
            NEON_FILTER_INSTANCE_U64X2_INIT_MULL_U32X2_WITH_CONST_LO_HI_LU2(
                accum_ref, offset_vec_h, ref_vec_u32, vif_filt_s0[0]);
            NEON_FILTER_INSTANCE_U64X2_INIT_MULL_U32X2_WITH_CONST_LO_HI_LU2(
                accum_dis, offset_vec_h, dis_vec_u32, vif_filt_s0[0]);
            NEON_FILTER_INSTANCE_U64X2_INIT_MULL_U32X2_WITH_CONST_LO_HI_LU2(
                accum_ref_dis, offset_vec_h, ref_dis_vec_u32, vif_filt_s0[0]);
            ...
            for (unsigned fj = 1; fj < fwidth; ++fj) {
                ...
                NEON_FILTER_UPDATE_ACCUM_U64X2_WITH_CONST_LO_HI_LU2(accum_ref, ref_vec_u32,
                                                                    vif_filt_s0[fj]);
                NEON_FILTER_UPDATE_ACCUM_U64X2_WITH_CONST_LO_HI_LU2(accum_dis, dis_vec_u32,
                                                                    vif_filt_s0[fj]);
                NEON_FILTER_UPDATE_ACCUM_U64X2_WITH_CONST_LO_HI_LU2(accum_ref_dis, ref_dis_vec_u32,
                                                                    vif_filt_s0[fj]);
            }

            NEON_FILTER_INSTANCE_U64X2_INIT_MULL_SHIFT_UNZIP_U32X4_LO_HI(
                mu1_sq_vec_l, vdupq_n_u64(2147483648), accum_mu1_0, accum_mu1_0, vdupq_n_s64(-32));
            NEON_FILTER_INSTANCE_U64X2_INIT_MULL_SHIFT_UNZIP_U32X4_LO_HI(
                mu1_sq_vec_h, vdupq_n_u64(2147483648), accum_mu1_1, accum_mu1_1, vdupq_n_s64(-32));

            NEON_FILTER_INSTANCE_U64X2_INIT_MULL_SHIFT_UNZIP_U32X4_LO_HI(
                mu2_sq_vec_l, vdupq_n_u64(2147483648), accum_mu2_0, accum_mu2_0, vdupq_n_s64(-32));
            NEON_FILTER_INSTANCE_U64X2_INIT_MULL_SHIFT_UNZIP_U32X4_LO_HI(
                mu2_sq_vec_h, vdupq_n_u64(2147483648), accum_mu2_1, accum_mu2_1, vdupq_n_s64(-32));

            NEON_FILTER_INSTANCE_U64X2_INIT_MULL_SHIFT_UNZIP_U32X4_LO_HI(
                mu1_mu2_sq_vec_l, vdupq_n_u64(2147483648), accum_mu1_0, accum_mu2_0,
                vdupq_n_s64(-32));
            NEON_FILTER_INSTANCE_U64X2_INIT_MULL_SHIFT_UNZIP_U32X4_LO_HI(
                mu1_mu2_sq_vec_h, vdupq_n_u64(2147483648), accum_mu1_1, accum_mu2_1,
                vdupq_n_s64(-32));
            ...
            NEON_FILTER_SHIFT_UNZIP_U64X2_TO_U32X4_LO_HI(accum_ref_dis_0, shift_vec_h,
                                                         xy_filt_vec_l);
            NEON_FILTER_SHIFT_UNZIP_U64X2_TO_U32X4_LO_HI(accum_ref_dis_1, shift_vec_h,
                                                         xy_filt_vec_h);

            int32x4_t sigma1_sq_vec_l =
                vreinterpretq_s32_u32(vsubq_u32(xx_filt_vec_l, mu1_sq_vec_l));
            int32x4_t sigma1_sq_vec_h =
                vreinterpretq_s32_u32(vsubq_u32(xx_filt_vec_h, mu1_sq_vec_h));

            int32x4_t sigma2_sq_vec_l =
                vreinterpretq_s32_u32(vsubq_u32(yy_filt_vec_l, mu2_sq_vec_l));
            int32x4_t sigma2_sq_vec_h =
                vreinterpretq_s32_u32(vsubq_u32(yy_filt_vec_h, mu2_sq_vec_h));

            int32x4_t sigma12_vec_l =
                vreinterpretq_s32_u32(vsubq_u32(xy_filt_vec_l, mu1_mu2_sq_vec_l));
            int32x4_t sigma12_vec_h =
                vreinterpretq_s32_u32(vsubq_u32(xy_filt_vec_h, mu1_mu2_sq_vec_h));
                           ...
                           int bpc, int scale)
{
    ...
    for (unsigned i = 0; i < h; ++i, i_dst_stride += stride_32) {
        ...
        for (; j < uiw7; j += 8, p_ref += 8, p_dis += 8) {
            NEON_FILTER_LOAD_U16X8_AND_MOVE_TO_U32X4_AND_SQR(ref_vec_16u, ref_vec_32u, ref_ref_vec,
                                                             p_ref);
            NEON_FILTER_LOAD_U16X8_AND_MOVE_TO_U32X4_AND_SQR(dis_vec_16u, dis_vec_32u, dis_dis_vec,
                                                             p_dis);

            NEON_FILTER_INSTANCE_U32X4_NO_INIT_MULL_U16X4_WITH_CONST_LO_HI(accum_f_ref, ref_vec_16u,
                                                                           vif_filt_s[0]);
            NEON_FILTER_INSTANCE_U32X4_NO_INIT_MULL_U16X4_WITH_CONST_LO_HI(accum_f_dis, dis_vec_16u,
                                                                           vif_filt_s[0]);

            NEON_FILTER_INSTANCE_U64X2_INIT_MULL_U32X2_WITH_CONST_LO_HI_LH(
                accum_f_ref_ref, add_shift_round_VP_sq_vec, ref_ref_vec, vif_filt_s[0]);
            NEON_FILTER_INSTANCE_U64X2_INIT_MULL_U32X2_WITH_CONST_LO_HI_LH(
                accum_f_dis_dis, add_shift_round_VP_sq_vec, dis_dis_vec, vif_filt_s[0]);
            NEON_FILTER_INSTANCE_U64X2_INIT_MULL_U32X2_LO_HI(
                accum_f_ref_dis_l, add_shift_round_VP_sq_vec, accum_f_dis_l, ref_vec_32u_l);
            NEON_FILTER_INSTANCE_U64X2_INIT_MULL_U32X2_LO_HI(
                accum_f_ref_dis_h, add_shift_round_VP_sq_vec, accum_f_dis_h, ref_vec_32u_h);
            ...
            for (unsigned fi = 1; fi < fwidth; ++fi, pp_ref += stride_16, pp_dis += stride_16) {
                NEON_FILTER_LOAD_U16X8_AND_MOVE_TO_U32X4_AND_SQR(ref_vec_16u, ref_vec_32u,
                                                                 ref_ref_vec, pp_ref);
                NEON_FILTER_LOAD_U16X8_AND_MOVE_TO_U32X4_AND_SQR(dis_vec_16u, dis_vec_32u,
                                                                 dis_dis_vec, pp_dis);

                NEON_FILTER_INSTANCE_U32X4_NO_INIT_MULL_U16X4_WITH_CONST_LO_HI(f_dis, dis_vec_16u,
                                                                               vif_filt_s[fi]);

                NEON_FILTER_UPDATE_ACCUM_U32X4_WITH_CONST_LO_HI(accum_f_ref, ref_vec_16u,
                                                                vif_filt_s[fi]);
                NEON_FILTER_UPDATE_ACCUM_U32X4_WITH_CONST_LO_HI(accum_f_dis, dis_vec_16u,
                                                                vif_filt_s[fi]);

                NEON_FILTER_ACCUM_LO_HI_LH_U64X2_WITH_CONST_LH(accum_f_ref_ref, ref_ref_vec,
                                                               vif_filt_s[fi]);
                NEON_FILTER_ACCUM_LO_HI_LH_U64X2_WITH_CONST_LH(accum_f_dis_dis, dis_dis_vec,
                                                               vif_filt_s[fi]);
            ...
            NEON_FILTER_OFFSET_SHIFT_STORE_U32X4(accum_f_ref_l, add_shift_round_VP_vec,
                                                 shift_VP_vec, buf.tmp.mu1 + j);
            NEON_FILTER_OFFSET_SHIFT_STORE_U32X4(accum_f_ref_h, add_shift_round_VP_vec,
                                                 shift_VP_vec, buf.tmp.mu1 + j + 4);

            NEON_FILTER_OFFSET_SHIFT_STORE_U32X4(accum_f_dis_l, add_shift_round_VP_vec,
                                                 shift_VP_vec, buf.tmp.mu2 + j);
            NEON_FILTER_OFFSET_SHIFT_STORE_U32X4(accum_f_dis_h, add_shift_round_VP_vec,
                                                 shift_VP_vec, buf.tmp.mu2 + j + 4);

            NEON_FILTER_SHIFT_UNZIP_STORE_U64X2_TO_U32X4_LO_HI(accum_f_ref_ref_l, shift_VP_sq_vec,
                                                               buf.tmp.ref + j);
            NEON_FILTER_SHIFT_UNZIP_STORE_U64X2_TO_U32X4_LO_HI(accum_f_ref_ref_h, shift_VP_sq_vec,
                                                               buf.tmp.ref + j + 4);

            NEON_FILTER_SHIFT_UNZIP_STORE_U64X2_TO_U32X4_LO_HI(accum_f_dis_dis_l, shift_VP_sq_vec,
                                                               buf.tmp.dis + j);
            NEON_FILTER_SHIFT_UNZIP_STORE_U64X2_TO_U32X4_LO_HI(accum_f_dis_dis_h, shift_VP_sq_vec,
                                                               buf.tmp.dis + j + 4);

            NEON_FILTER_SHIFT_UNZIP_STORE_U64X2_TO_U32X4_LO_HI(accum_f_ref_dis_l, shift_VP_sq_vec,
                                                               buf.tmp.ref_dis + j);
            NEON_FILTER_SHIFT_UNZIP_STORE_U64X2_TO_U32X4_LO_HI(accum_f_ref_dis_h, shift_VP_sq_vec,
                                                               buf.tmp.ref_dis + j + 4);
        }
        ...
        for (; j < uiw7; j += 8, pMul1 += 8, pMul2 += 8, pDis += 8, pRef += 8, pRefDis += 8) {
            ...
            NEON_FILTER_INSTANCE_U64X2_INIT_MULL_U32X2_WITH_CONST_LO_HI_LU2(
                accum_ref, add_shift_round_HP_vec, ref_vec_u32, vif_filt_s[0]);
            NEON_FILTER_INSTANCE_U64X2_INIT_MULL_U32X2_WITH_CONST_LO_HI_LU2(
                accum_dis, add_shift_round_HP_vec, dis_vec_u32, vif_filt_s[0]);
            NEON_FILTER_INSTANCE_U64X2_INIT_MULL_U32X2_WITH_CONST_LO_HI_LU2(
                accum_ref_dis, add_shift_round_HP_vec, ref_dis_vec_u32, vif_filt_s[0]);
            ...
            NEON_FILTER_SHIFT_UNZIP_U64X2_TO_U32X4_LO_HI(accum_ref_dis_0, shift_vec_HP,
                                                         xy_filt_vec_l);
            NEON_FILTER_SHIFT_UNZIP_U64X2_TO_U32X4_LO_HI(accum_ref_dis_1, shift_vec_HP,
                                                         xy_filt_vec_h);
        ...
        if (j != w) {
            VifResiduals residuals = vif_compute_line_residuals(s, j, w, scale);
            accum_num_log += residuals.accum_num_log;
            accum_den_log += residuals.accum_den_log;
            accum_num_non_log += residuals.accum_num_non_log;
            accum_den_non_log += residuals.accum_den_non_log;
