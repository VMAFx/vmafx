                                                   ...
                                                   int j_end)
{
    ...
    if (count <= 0)
        return;
    const int full_count = vmaf_floorn(count, AVX512_STEP);
    ...
    for (int j = 0; j < full_count; j += AVX512_STEP) {
    ...
    if (remaining > 0) {
        const __mmask16 mask = (__mmask16)((1U << remaining) - 1U);
        __m512 sum = _mm512_setzero_ps();
        for (int k = 0; k < filter_width; k++) {
            const __m512 g = _mm512_maskz_loadu_ps(mask, src + full_count + k);
            sum = _mm512_fmadd_ps(f[k], g, sum);
        }
        _mm512_mask_storeu_ps(dst + full_count + radius, mask, sum);
    }
}
...
static void convolution_f32_avx512_horizontal(const float *RESTRICT filter, int filter_width,
                                              const float *RESTRICT tmp, float *RESTRICT dst,
                                              int width, int height, int dst_stride)
{
    const int radius = filter_width / 2;
    const int tmp_stride = vmaf_ceiln(width, AVX512_STEP);
                              ...
                              int width, int height, int src_stride, int dst_stride)
{
    ...
    convolution_f32_avx512_horizontal(filter, filter_width, tmp, dst, width, height, dst_stride);
}
