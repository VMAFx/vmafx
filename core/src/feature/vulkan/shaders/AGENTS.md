...
build line invokes `glslc --target-env=vulkan1.3` per shader;
precision-sensitive shaders (`psnr_hvs.comp`, `float_vif.comp`,
`ssimulacra2_{blur,ssim,xyb}.comp`) compile with `-O0` to disable
SPIR-V-optimizer reassociation/FMA-contraction (see `psnr_hvs_strict_shaders`
list in `meson.build`); all others compile with `-O`
