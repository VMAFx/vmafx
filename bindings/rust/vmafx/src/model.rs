...
impl Model {
    ...
    ///
    /// Returns [`Error::Nul`] if `path` contains an interior NUL byte and
    /// [`Error::Libvmaf`] (or one of the mapped variants) if libvmaf
    /// rejects the file.
    pub fn from_path<P: AsRef<Path>>(path: P) -> Result<Self> {
        ...
        let rc = unsafe { vmaf_model_load_from_path(&mut model, &mut cfg, c_path.as_ptr()) };
    ...
    #[must_use]
    pub fn as_ptr(&self) -> *mut RawModel {
        self.inner
    }

    /// Same as [`as_ptr`](Self::as_ptr) but takes `&mut self`.
    #[must_use]
    pub fn as_mut_ptr(&mut self) -> *mut RawModel {
        self.inner
    }
}
