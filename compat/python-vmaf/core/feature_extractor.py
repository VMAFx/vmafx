...
class FeatureExtractor(Executor):
    ...
    def get_score_key(cls, atom_feature):
        return "{type}_{atom_feature}_score".format(type=cls.TYPE, atom_feature=atom_feature)
...
class VmafexecFeatureExtractorMixin(object):
    ...
    @classmethod
    def _discover_feature_exact(cls, frame, i_feature, feature, feature_scores, feature_nicknames):
...
class VifFrameDifferenceFeatureExtractor(FeatureExtractor):
    ...
    def _generate_result(self, asset):
        ...
        ExternalProgramCaller.call_vifdiff_feature(
            yuv_type, ref_path, dis_path, w, h, log_file_path, logger
        )
...
class SpeedChromaFeatureExtractor(VmafexecFeatureExtractorMixin, FeatureExtractor):
    """Full-reference SpEED-QA extractor applied on the chroma (U/V) channels.

    Ported from Netflix/vmaf ``python/vmaf/core/feature_extractor.py``.
    Wraps the ``speed_chroma`` C feature extractor via the vmafexec CLI.
...
class SpeedTemporalFeatureExtractor(VmafexecFeatureExtractorMixin, FeatureExtractor):
    """Full-reference SpEED-QA extractor applied on luma frame-differences.

    Ported from Netflix/vmaf ``python/vmaf/core/feature_extractor.py``.
    Wraps the ``speed_temporal`` C feature extractor via the vmafexec CLI.
    Research-0732 audit (PR #22) identified this class as missing from
    ``compat/python-vmaf/``; restored here per that audit.
    """
