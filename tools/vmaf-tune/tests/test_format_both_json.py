...
The compare emission path is covered here. The shared artifact writer is
covered in ``test_report.py`` and the report subcommand is exercised through
the public CLI in ``test_cli_subcommands.py``.

No ffmpeg / vmaf binaries required — the compare path uses a fake predicate
and a stubbed source probe.
"""
