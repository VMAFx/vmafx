...
func BuildVMAFCommand(ref, distorted, jsonOutput string, p YUVScoreParams) []string {
	...
	argv := []string{
		...
		"--model", vmafmodel.CLIArgumentOrDefault(p.Model),
		"--json",
		"--output", jsonOutput,
	}
	if p.Backend != "" {
...
func YUVScoreFunc(p YUVScoreParams) ScoreFunc {
	return func(ref, distorted string) (float64, error) {
		return scoreYUV(p, ref, distorted)
	}
}

func scoreYUV(p YUVScoreParams, ref, distorted string) (float64, error) {
	workDir := p.WorkDir
	if workDir == "" {
		workDir = os.TempDir()
	}
	...
	scoreTarget, decoded, err := decodeScoreTarget(p, distorted, workDir)
	if err != nil {
		return 0, err
	}
	if decoded != "" {
		defer removeScoreArtifact(decoded, "decoded distorted YUV")
	}

	jsonPath, err := createScoreOutput(workDir)
	if err != nil {
		return 0, err
	}
	defer removeScoreArtifact(jsonPath, "score temp")

	return executeVMAFScore(BuildVMAFCommand(ref, scoreTarget, jsonPath, p), jsonPath)
}

func decodeScoreTarget(p YUVScoreParams, distorted, workDir string) (string, string, error) {
	if rawYUVSuffixes[strings.ToLower(filepath.Ext(distorted))] {
		return distorted, "", nil
	}
	decoded := filepath.Join(workDir,
		strings.TrimSuffix(filepath.Base(distorted), filepath.Ext(distorted))+".decoded.yuv")
	release := acquire(p.DecodeSem)
	defer release()
	if err := DecodeToRawYUV(distorted, decoded, p.PixFmt, p.FFmpegBin, p.DurationS); err != nil {
		return "", "", err
	}
	return decoded, decoded, nil
}

func createScoreOutput(workDir string) (string, error) {
	jsonOut, err := os.CreateTemp(workDir, "vmafx-tune-score-*.json")
	if err != nil {
		return "", fmt.Errorf("create score temp: %w", err)
	}
	jsonPath := jsonOut.Name()
	if err := jsonOut.Close(); err != nil {
		removeScoreArtifact(jsonPath, "unclosed score temp")
		return "", fmt.Errorf("close score temp: %w", err)
	}
	return jsonPath, nil
}

func removeScoreArtifact(path, description string) {
	if err := os.Remove(path); err != nil && !errors.Is(err, os.ErrNotExist) {
		slog.Warn("bisect: remove "+description, "error", err, "path", path)
	}
}

func executeVMAFScore(argv []string, jsonPath string) (float64, error) {
	ctx := context.Background()
	cancel := func() {}
	...
	if err != nil {
		return 0, fmt.Errorf("read vmaf output %q: %w", jsonPath, err)
	}
