...
1. **Manifest authority (`requirements/locks/manifest.json`)**:
   ...
   - `install_aliases` are exact, reviewed objects bound to explicit repo-relative consumer paths and context (preventing unrelated workflows from consuming alias paths). Consumer identity uses exact separator-normalized equality: `evil/scripts/setup/ubuntu.sh` never inherits authority from `scripts/setup/ubuntu.sh`. Traversal, Windows drive/UNC paths, duplicate JSON keys, and unreferenced/dead aliases are rejected. Never replace exact equality with basename/suffix heuristics; a lookalike path outside the manifest is untrusted.
