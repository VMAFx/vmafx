...
Add `speed_chroma` and `speed_temporal` to extraction scripts (`ai/data/feature_extractor.py`
`FULL_FEATURES`, `bvi_dvc_to_full_features.py`) as HDR-model preparation (ADR-0559).
Feature coverage audit research digest at `docs/research/feature-coverage-audit-2026-05-18.md`.
...
- **Tiny-AI Netflix corpus training scaffold (2026-05-20 iteration, ADR-0640)**: adds
  Research Digest 0615 updating the EfficientVMAF (CVPR 2024) and IQA-PyTorch
  distillation survey; extends the architecture alternatives table with the
  learned feature-reweighting option; updates `docs/ai/training-data.md`
  cross-references.  No training run; no Netflix golden assertions modified.
  Architecture selection and the actual training run remain deferred to a
  follow-up PR.
...
signal families from extracted parquet/JSONL tables.


CHUG feature extraction now preserves normalized ffprobe HDR/display metadata
on each emitted training row for both the reference and distorted clip.


CHUG feature extraction now emits decoded-luma blur, noise, grain, and contrast
proxy fields for reference/distorted clips plus distorted-minus-reference deltas.


Add `chug-hdr-display-v1` MOS-head training support so CHUG HDR runs can ingest
target display profiles and record the normalized panel context in manifests.


Add `ai/scripts/materialize_saliency_features.py` to append saliency aggregates
to JSONL/parquet AI feature tables before predictor and MOS-head retrains.


Add `ai/scripts/materialize_second_opinion_features.py` for joining NR/MOS
second-opinion scorer outputs onto refreshed AI feature tables.


- Add `scripts/dev/project_modernization_audit.py`, a read-only JSON/Markdown
  scanner that ranks repo modernization gaps, local state rows, smoke-model
  registry entries, and large AI script-family clusters.


Add shared AI run-manifest provenance helpers and MOS-head `run_provenance` sidecar blocks.


Add `ai/scripts/materialize_mos_labels.py` for joining subjective MOS labels
onto refreshed AI feature tables before real MOS-head training.


FR regressor and vmaf_tiny export sidecars now record shared `run_provenance`
metadata, and v1/v2 metrics JSON files keep the same provenance for failed or
no-export runs.


Tiny-VMAF eval reports and ensemble-seed validation verdicts now record shared
`run_provenance` metadata, including input paths, run arguments, and report or
verdict targets.
...
- `vmaf-tune report` HTML and Markdown profile cards now include a Quick
  takeaways section that summarizes the winning row/frontier, failed encoder
  count, ladder span, and per-shot CRF spread before the detailed charts.


Predictor-v2 real-corpus reports now include shared `run_provenance` metadata,
recording the trainer entrypoint, argv, parsed arguments, corpus roots/files,
and report target for each gate run.


AI FULL_FEATURES table builders now emit replayable manifest sidecars with
shared `run_provenance` for K150K extraction, refreshed parquet combination,
and CHUG/K150K metadata enrichment outputs.


vmaf_tiny v2/v3/v4/v5 training stats JSON files now include shared
`run_provenance` metadata for the trainer entrypoint, argv, parsed
hyperparameters, parquet input(s), checkpoint target, and stats target.
...
- AI materializer and signal-mix audit JSON outputs now record shared
  `run_provenance`; saliency materialization also supports `--audit-json`.


AI legacy corpus/extraction scripts now emit replayable manifest sidecars with shared run provenance, and the BVI-DVC JSONL adapter emits current vmaf-tune v3 row defaults.


- Fresh `fr_regressor_v2_ensemble_v1_seed*` exports now include ADR-0661
  `run_provenance` in their per-seed sidecars.


- Ensemble LOSO training reports (`loso_seed{N}.json`) now include ADR-0661
  `run_provenance` with corpus input, argv, parsed training arguments, and the
  report target.


- Add `ai/scripts/export_u2netp_mirror.py`, an audited-checkout U2NetP
  mirror exporter that writes ONNX plus a provenance manifest without
  committing model binaries.


`ai/scripts/materialize_saliency_features.py` now accepts temporal saliency
reducer controls and records model/reducer metadata on newly materialized
feature-table rows.


- `vmaf-train` JSON reports now include ADR-0661 `run_provenance` for
  normalization checks, profiling, learned-filter audits, INT8 quantization
  drift, cross-backend diffs, and model-quality bisection.


`ai/scripts/feature_correlation.py --out` now records ADR-0661
`run_provenance` in its JSON report so signal-mix rankings carry replayable
source-parquet and argument context.


Add `ai/scripts/batch_materialize_saliency_features.py`, a manifest-driven
batch runner for saliency feature materialization with per-table audits and a
provenance-backed batch report.


`ai/scripts/phase3_subset_sweep.py --out` now records ADR-0661
`run_provenance` in its JSON report so Phase-3 model-selection sweeps preserve
their source parquet and sweep arguments.


Add `ai/scripts/batch_materialize_second_opinion_features.py`, a manifest-driven
batch runner for joining external second-opinion score sidecars across multiple
feature tables with per-table audits and a provenance-backed batch report.


Add `ai/scripts/batch_materialize_mos_labels.py`, a manifest-driven batch
runner for joining subjective MOS labels across multiple feature tables with
per-table audits and a provenance-backed batch report.
...
MOS corpus JSONL adapters now emit replay manifest sidecars with source paths,
row counters, effective ingest config, and shared AI run provenance.


`ai/scripts/calibrate_phase_f_recipes.py` now records shared
`run_provenance` in calibrated `vmaf-tune auto` recipe JSON so regenerated
thresholds carry the corpus, row-cap, argv, and output-target context.
...
Add a shared AI run-manifest helper and Claude workflow for standalone artifact
sidecars, plus replay manifests for legacy extractor/cache and quantization
helpers.


`ai/scripts/train_fr_regressor_v2_ensemble.py` now records ADR-0661
`run_provenance` inside refreshed `fr_regressor_v2_ensemble_v1.json` manifests,
linking the top-level ensemble runtime entry point to its corpus, member ONNX
outputs, registry target, CLI arguments, and manifest path.


- Added ADR-0661 `run_provenance` to C2/C3, FastDVDnet, and TransNet exporter
  sidecars so refreshed DNN feature-model artifacts record their checkpoint or
  upstream-weight inputs, exporter arguments, ONNX output, sidecar output, and
  registry target.


Add shared AI CLI helpers for batch manifest runners and switch the saliency,
second-opinion, and MOS-label batch materializers to the common parser/raw-argv
boilerplate.


- Added ADR-0661 `run_provenance` to saliency-student v1/v2 training metrics so
  refreshed DUTS runs record their corpus root, training arguments, ONNX output,
  metrics output, and original argv.


- Added optional JSON reports with ADR-0661 `run_provenance` to the
  `validate_vmaf_tiny_v2.py`, `validate_vmaf_tiny_v3.py`, and
  `validate_vmaf_tiny_v4.py` smoke validators.


- Added optional ADR-0661 JSON reports to the PTQ/QAT quantisation scripts and
  the `measure_quant_drop.py` CI gate.
...
- Added optional ADR-0661 JSON reports to the model-registry and
  saliency-student validation scripts.


- CHUG split manifests and HDR metadata audit JSONs now include ADR-0661
  `run_provenance` so HDR MOS feature-table evidence records the extractor
  command, inputs, and output targets.
...
KoNViD and BVI-DVC full-feature parquet exporters now emit replayable
`<out>.manifest.json` sidecars with feature order, source/cache/model inputs,
...
- **Research digest** — vmaf-tune profile-report renderer audit (`T-VMAFTUNE-PROFILE-REPORT-AUDIT`):
  `docs/research/vmaftune-profile-report-audit-2026-05-27.md` documents 10 findings across
...
- **MCP `probe_backend` tool (ADR-0608)**: runtime health check for GPU backends — distinguishes "compiled in" from "driver present and functional" by running a 1-frame score on a synthetic 32×32 YUV. Returns `compiled_in`, `runtime_healthy`, `latency_ms`, `score`, and `error`.
- **MCP `vmaf_version` tool (ADR-0608)**: returns the local vmaf binary path, version string (from `--version`), and build flags dict (cpu/cuda/sycl/vulkan/hip/metal).
- **MCP `vmaf_score_encoded` tool (ADR-0608)**: score encoded video files (MP4/MKV/Y4M/WebM/…) directly — geometry is probed via `ffprobe`, both inputs decoded in parallel via `ffmpeg`, then scored via the standard `vmaf_score` pipeline. No manual width/height/pixfmt entry required.
...
- **AGENTS.md per-package coverage audit + backfill.**
  Audits every code-bearing directory under `core/src/`, `tools/`,
  ...
  `core/src/feature/{x86,arm64}/`, the IQA scalar reference
  (`core/src/feature/iqa/`) and Xiph third-party reference
  (`core/src/feature/third_party/xiph/`), the per-feature GPU
  kernel directories (`core/src/feature/{cuda,sycl,vulkan}/` plus
  `core/src/feature/vulkan/shaders/`), the SVE2 HWCAP2 fork-local
  fallback under `core/src/arm/`, the MCP scaffold contract
  (`core/src/mcp/`), the fork-original ensemble training kit
...
### Added
- 62 new ai/src branch-coverage tests across 9 modules (aiutils, audit, cross_backend, op_allowlist, parquet_utils, profile, run_manifest, subprocess_utils, validate_norm).
...
- Add `test_cambi_vulkan` smoke test: verifies extractor registration,
  default-parameter parity with the CPU scalar cambi.c, and end-to-end
  init/extract/close on a flat 576x324 frame (ADR-0456 gap 1).
...
Added a local-only CHUG UGC-HDR MOS-corpus ingestion adapter,
reference-aligned feature materialiser, and training-discovery synthesis
tooling for HDR model experiments.
...
- **docs(backends)**: add `docs/backends/context-api-contract.md` codifying the
  ...
  opaque-handle accessor pattern, and a new-backend checklist (ADR-0486).
...
- Add the `dists_sq` tiny-AI full-reference feature extractor with an in-tree
  smoke ONNX checkpoint and model card.


- **`--dnn-ep` CLI flag and `vmaf_tiny_v3_int8` registry promotion.**
  Two additive wiring fixes identified in the DNN-EP audit:
  (1) Add `--dnn-ep <ep>` to the `vmaf` CLI (`core/tools/cli_parse.c`)
  ...
  ORT EP selection in `core/src/dnn/ort_backend.c` is unchanged.
...
- **Governance**: added top-level `GOVERNANCE.md` (BDFL model,
  decision-making, releases) and `MAINTAINERS.md` (current
  maintainer list, subtree ownership, onboarding / stepping-down
  process). Extended `.github/CODEOWNERS` with per-subtree rows
  for `/ai/`, `/mcp-server/`, `/cmd/`, `/deploy/`, `/docker/`,
  `/dev/`, `/compat/`, `/ffmpeg-patches/`, `/scripts/`,
  `/tools/`, `/docs/adr/`, `/changelog.d/`, and the new
  governance files. `CONTRIBUTING.md` now documents branch
  naming, the ADR-0108 deliverables gate, the ADR allocator, and
  links to the new governance files. See ADR-0901.
...
- HDR/UGC dataset license + access audit
  ([Research-0136](../docs/research/0136-hdr-ugc-dataset-license-audit-2026-05-15.md))
  evaluating 13 candidate corpora flagged in Audit Slice C.7. 6 datasets
  classified ACTIONABLE-NOW (Beyond8Bits, HDRSDR-VQA, LIVE HDR Database,
  IPI-MobileHDRVQA, HDR-VDC, plus the already-active CHUG); 5 BLOCKED on
...
- **HIP `float_psnr_hip` — first real AMD GPU kernel
  ...
  `core/src/feature/hip/float_psnr/float_psnr_score.hip` implements
...
Added name-lookup registration assertions in `core/test/test_metal_smoke.c`
...
invariant note to `core/src/feature/hip/AGENTS.md` and
`core/src/feature/metal/AGENTS.md` requiring every new backend extractor
...
Add `integer_vif_vulkan` feature extractor source (`core/src/feature/vulkan/integer_vif_vulkan.c`),
...
Add `test_model_collection_api` — C unit tests covering the four model-collection
public API entry points (`vmaf_model_collection_load`,
`vmaf_use_features_from_model_collection`, `vmaf_score_at_index_model_collection`,
`vmaf_score_pooled_model_collection`) that had zero coverage per the
...
Add `docs/metrics/motion.md` — dedicated reference page for the `motion`,
`motion_v2`, and `float_motion` feature extractors. Covers options tables,
output feature names, output ranges, input format constraints, and a
...
Added `core/test/test_cuda_motion3_parity.c`: cross-backend parity test
...
(ADR-0617), and content-aware classifier (ADR-0618). Includes research digests,
ADRs with alternatives tables and dependency graphs, and a sequencing overview
at `docs/development/netflix-grade-encoding-pipeline-roadmap-2026-05-19.md`.
...
Added `ai/scripts/eval_saliency_per_mb.py`, a dependency-light saliency
mask evaluator that reports macro / micro IoU after reducing predicted
and ground-truth masks to encoder block grids.
...
- **CUDA `psnr` chroma extension** (`psnr_cuda` now emits `psnr_y`,
  ...
  the CPU contract at `core/src/feature/integer_psnr.c`. Cross-backend
...
Add `test_version` unit test asserting `vmaf_version()` returns a non-null,
non-empty, printable string. Closes a coverage gap noted in
...
`vmaf-roi-score --saliency-model` now materialises saliency-masked
8-bit planar YUV (`yuv420p`, `yuv422p`, `yuv444p`) and scores it
instead of returning the scaffold exit path.
...
`vmaf-tune auto` now marks one selected plan cell and records the decision in
`metadata.winner`, giving downstream scripts a deterministic Phase F target
before full encode execution lands.


- **vmaf-tune**: Add Phase-B bisect sample-clip support and
  `vmaf-tune compare --sample-clip-seconds`, aligning FFmpeg `-ss` /
  `-t` with libvmaf `--frame_skip_ref` / `--frame_cnt` and bitrate
  normalisation.


Added `vmaf-tune benchmark`, a read-only Phase-G report over existing
Phase-A JSONL corpora that ranks encoders at a matched target VMAF and
emits markdown, JSON, or CSV.
...
`vmaf-tune recommend-saliency --encoder libaom-av1` now emits the fork
FFmpeg `-qpfile` ROI bridge instead of falling back to a plain encode.


`vmaf-tune` now registers a `libvpx-vp9` codec adapter with VP9 CRF mode,
shared preset-axis mapping, version-banner parsing, and FFmpeg-native
two-pass support.
...
- CI CUDA pin unchanged at `13.2.0` in
  `.github/workflows/libvmaf-build-matrix.yml` (already pinned by prior SDK
  audit; container now matches).
...
Refresh `fr_regressor_v1` from the 2026-05-20 Netflix full-feature table,
updating the ONNX, sidecar, registry sha256, and model card with the new LOSO
metrics.
...
Refactored AI evaluation, calibration, and diagnostic scripts to share the
standard script bootstrap and CLI helper utilities while preserving their report
schemas.


Refactored AI full-feature and corpus-prep scripts to use the shared script
bootstrap and CLI helper utilities, and derived the BVI-DVC feature extractor
set from the canonical AI feature registry.


### AI

- Migrated the active materializer and signal-audit scripts to the shared
  `aiutils.cli_helpers` parser and raw-argv helpers.
...
- Migrated the QAT, PTQ, quant-drop, and tiny-DNN export scripts to the shared
  script-bootstrap and CLI helper pattern.
...
- Migrated the versioned `vmaf_tiny` train/export/validation scripts to the
  shared script-bootstrap and CLI helper pattern.


### AI

- Migrated the VMAF tiny LOSO and multi-seed evaluation scripts to the shared
  script-bootstrap and CLI helper pattern.
...
- Migrated the active materializer and signal-audit scripts to the shared
  `ai/scripts/_script_bootstrap.py` import bootstrap.
...
package-local JSON helper for NaN/Infinity-to-null coercion.
...
Replace stale `TODO` for `adm_p_norm` integration in `integer_adm.c` with a
structured `DEFERRED` block that documents why the exponent is hard-coded
(model-training coupling) and what the parameterised form would look like.
...
ADR-0325 (KonViD-150k corpus ingestion): flip index row in `docs/adr/README.md`
from Proposed → Accepted, matching the ADR file status set 2026-05-15 when the
corpus materialized at `.workingdir2/konvid-150k/`.
...
- **docs**: bulk-flip ADR Status `Proposed` → `Accepted` for 15 ADRs
  confirmed implemented on master by the 2026-05-15 audit. Flipped:
  ADR-0128 (embedded MCP server), ADR-0318 (ensemble retrain harness),
  ADR-0326 (vmaf-tune Phase B bisect), ADR-0332 (agent worktree drift
  guard), ADR-0336 (KonViD MOS head v1), ADR-0339 (av1_videotoolbox
  adapter), ADR-0340 (multi-corpus aggregation), ADR-0345 (cambi
  CUDA+SYCL; HIP deferred), ADR-0346 (FR-from-NR adapter), ADR-0347
  (sanitizer matrix), ADR-0361 (Metal backend scaffold), ADR-0368
  (external-bench wrapper), ADR-0393 (fr_regressor_v2 probabilistic
  scaffold), ADR-0412 (u2netp mirror scaffold), ADR-0417 (tiny-AI
  Netflix training scaffold). ADR-0325 was already Accepted via
  PR #847 and was skipped.
...
- **chore(adr):** Remove 3 orphaned `docs/adr/*.md.stub` reservation files
  (0729, 0830, 0846) whose corresponding ADRs were committed or whose
  reservations were never fulfilled. Also purge 110 stale `.git/adr-claims/`
  side-pointers older than 24 hours whose ADRs have since landed on master.
  Two live stubs (0833, 0847) are preserved.
...
- **CI:** Per-lane wall-clock optimizations for the three slowest CI lanes
  ...
  ~30–60 s per run; (2) Ubuntu Vulkan build caches `core/subprojects/packagecache/`
...
- **CodeQL config**: globally suppress the `cpp/poorly-documented-function`
  ...
  currently-open alerts in `core/src/` close on the next scan. See
...
- **Dedup duplicate-NNNN ADRs (bookkeeping).** Renumbered ten ADR files
  ...
  headings, in-tree citations (`docs/`, `core/src/`, `ai/`,
...
- docs: API reference and backends pages now include Vulkan/HIP/Metal headers
  ...
  `core/include/libvmaf/libvmaf_{vulkan,hip,metal}.h` and
  `core/src/feature/metal/*.mm` actually ship.
...
Replace stale `libvmaf/` source-tree path refs with `core/` across 75 non-ADR doc
files; fix 9 dangling ADR fragment link targets caused by dedup-sweep renumbering;
add 26 ADR index fragments for ADRs 0683–0717 (merge-train batch); regenerate
`docs/adr/README.md` via `scripts/docs/concat-adr-index.sh --write`.


Added research digest 0734: R610.43.02 driver changelog audit. Confirms changelog.html
still 404 at NVIDIA XFree86 index. All confirmed R610 changes are display/graphics-layer
only (Vulkan extensions, DRM color pipeline, FP16 EGL, DMABUF mmap). No CUDA/UVM/
scheduler changes found; no fork code action required.
...
Research-0733: identified rclone FUSE page-cache bypass via eBPF as the highest-value
Phase 4b.6 optimization target. Projected 15–40% job wall-time reduction on warm-cache
nodes for 1080p60 clips. Phased implementation plan (4b.6.a–4b.6.d) established.
...
Add `docs/usage/env-vars.md` — a single, code-audited reference page for all
30+ environment variables across the C library, AI scripts, MCP server, and
Go binaries (ADR-0841).  Ghost vars removed from `docs/server/node.md`;
`docs/server/operator.md` created for the operator binary.  Dispatch-knob
paragraphs added to both CUDA and SYCL backend overviews.
...
- **docs(ai)**: Refresh the `fr_regressor_v2` model cards so the main
  card describes the registry-promoted production checkpoint and the
  older `fr_regressor_v2_codec_aware` card is clearly marked as a
  superseded design note, not a still-planned ONNX artefact.
...
`vmaf_gpu_dispatch_parse_env` into `core/src/gpu_dispatch_parse.h`,
...
shared `memset`-backed macro in `core/src/kernel_lifecycle_common.h`) instead
...
KonViD-150k ingestion now accepts the staged `k150ka_scores.csv` /
`k150kb_scores.csv` plus extracted-MP4 layout when no `manifest.csv`
is present.
...
Allow embedded MCP `compute_vmaf` requests to score YUV420p 10/12/16-bit
inputs through a new optional `bitdepth` argument, while preserving the
8-bit default.
...
- **Metal (Apple Silicon) compute backend runtime (T8-1b)
  ...
  [`core/src/metal/common.mm`](../core/src/metal/common.mm)
  ...
  gate),
  [`core/src/metal/picture_metal.mm`](../core/src/metal/picture_metal.mm)
  (MTLBuffer allocator using `MTLResourceStorageModeShared` —
  zero-copy unified memory on Apple Silicon), and
  [`core/src/metal/kernel_template.mm`](../core/src/metal/kernel_template.mm)
  ...
  of every header in `core/src/metal/` and out of every
  consumer TU under `core/src/feature/metal/` (header-purity
...
- **docs(models)**: Refresh the human-facing model overview and FAQ so
  they describe the shipped `model/tiny/` ONNX registry instead of
  claiming tiny-AI weights are still future work. The docs now point
  operators to the registry, per-model cards, inference flow,
  signature verification, and smoke-vs-production row distinction.
...
- **`nightly.yml` + `fuzz.yml` triage — gates stay on, bugs documented for
  ...
  `div_lookup_generator` (`core/src/feature/integer_adm.h:32-38`) where
  ...
  `y4m_input_fetch_frame` (`core/tools/y4m_input.c:877`) on negative-
...
- Planning hygiene sweep (2026-05-15): flipped 8 ADRs from Proposed to
  Accepted whose implementation has fully landed (ADR-0331, ADR-0341,
  ADR-0353, ADR-0365, ADR-0394, ADR-0396, ADR-0397, ADR-0403);
  removed 3 duplicate `Netflix#955` rows + duplicate `T-VK-1.4-BUMP`
  + stale `T-VK-VIF-1.4-RESIDUAL-NVIDIA-DEFERRED` Open row from
  `docs/state.md`; updated `CLAUDE.md §5` to document `.workingdir/`
  and §12 r11 to reference `changelog.d/` per ADR-0221.
...
- All in-tree references to `lusoris/vmaf` (GitHub URLs, repository slugs, GHCR
  ...
  registry convention. 113 files updated, 279 occurrences replaced. PR:
...
Added prominent banner to CLAUDE.md, AGENTS.md, and the PR template identifying
`VMAFx/vmafx` as the active repository and `lusoris/vmaf` as archived, preventing
agents from targeting the wrong remote with gh commands.
...
### Research digest header ID normalization

Normalized 26 research digest files under `docs/research/` so that the `# Research-NNNN` header ID matches the filename ID (e.g., `0033-foo.md` now has `# Research-0033`). Filenames are the authoritative identifier for cross-links; this ensures consistency.
...
`vmaf-roi-score` saliency-mask materialisation now supports
...
- Sanitizer matrix (ASan / UBSan / TSan in
  ...
  `core/test/meson.build` carries a `suite: 'unit'` tag — every leg
  ...
  pattern in `core/test/test.h` + ~50 `test_*.c` files; library
...
`testdata/bench_perf.py` now supports CLI/env configuration, backend
selection, `--list-tests`, and `--dry-run` so the FFmpeg lavfi performance
harness is usable outside one local workstation.


- **docs/tiny-ai**: Refresh FastDVDnet and TransNet V2 roadmap / feature
  limitation text so the user-facing docs reflect shipped real upstream
  weights instead of the old smoke-placeholder status.


LPIPS and DISTS-Sq now accept planar 10/12/16-bit YUV inputs by normalising
them into the existing RGB8 ImageNet tensor contract before ONNX inference.
...
`vmaf-perShot` now accepts planar 4:2:2 and 4:4:4 YUV inputs by counting the
selected chroma layout while keeping the detector and CRF prior luma-only.
...
`vmaf-tune auto` non-smoke plans now populate cells from the existing
predictor path instead of the former fixed CRF-23 placeholder, including
the `prediction_source` marker for JSON consumers.


`vmaf-tune auto` non-smoke planning now uses a testable source probe seam
for ffprobe geometry, ffprobe duration, and HDR detection, and its CLI/docs no
longer describe the path as an unimplemented scaffold.
...
- Feed `vmaf-tune recommend-saliency` with chroma-aware RGB tensors instead of
  luma-replicated input, and replace the SSIMULACRA2 metric stub page with
...
Research digest (Research-0731) documenting the VMAFx GitHub org migration plan:
monorepo split audit (Options A/B/C), cutover execution steps, branch-protection
export, upstream-tracking skeleton, open PR transfer inventory, and rollback plan.
Recommendation: stay monorepo for cutover day; defer models-only split (Option C)
as a follow-on PR. References ADR-0686.


- **Project rebrand: VMAFX** — adopts new project name "VMAFX" (VMAF
  eXperimental/eXtended) for the fork. `libvmaf.so` ABI preserved.
  Sweep PRs follow per the umbrella ADR. Fork-added code dual-licensed
  as `BSD-3-Clause-Plus-Patent OR MIT` (training-data ingestion).
...
- macOS-arm contributor build no longer fails with *"SVE vector
  ...
  the runtime detection in `core/src/arm/cpu.c` has always
  ...
  `core/src/meson.build`, mirroring the runtime gate. Linux
...
Attached tiny-AI models now record every scalar ONNX output under stable per-output feature names instead of rejecting multi-output graphs.


Fix `tools/external-bench` fork wrapper payloads so `fork-fr-regressor` and
`fork-nr-metric` aggregate instead of being skipped as invalid competitor
schema.


- Tighten the project-modernization audit so historical `NotImplementedError`
  prose, Python exception handlers, and documented `-ENOSYS` disabled-build
  contracts no longer appear as fresh scaffold gaps.
...
Fix the ADR collision guard so ADR-bearing parent PRs ignore descendant
stacked drafts that intentionally contain the parent's ADR files.


- Fix draft PR merge-train gating so skipped draft-era required checks cannot
  satisfy branch protection, and avoid ADR self-collision failures after fast
  merges.
...
- Replace hardcoded `/home/kilian/dev/vmaf` and `/home/kilian/datasets` paths with
  portable alternatives in `testdata/bench_all.sh`, `ai/data/phase_f_recipes_calibrated.json`,
  `ai/scripts/train_saliency_student.py`, and `ai/scripts/train_saliency_student_v2.py`.
  Closes T-PYTHON-PERMUTATION-IMPORTANCE-HARDCODED-PATH (state.md).
...
- AI corpus and materializer JSONL row writers now share a strict serializer so
  non-finite diagnostics are emitted as standard JSON `null`.


- AI legacy cache/report/stdout JSON writers now use the shared strict JSON
  boundary so non-finite diagnostics serialize as standard JSON `null`.


- `ai/lpips_export.py` now accepts the documented `--sidecar` path, keeps
  `--out` as an alias, and records strict JSON run provenance in the sidecar.
...
- **MCP tool catalogue sync** (`docs/mcp/`): updated `docs/mcp/index.md` to
  list all 15 MCP tools (8 added by ADR-0613 and ADR-0608 were missing from
  the tool catalogue table and the capabilities summary). Fixed
  `docs/mcp/tools.md` to remove a duplicate "Cross-tool error conventions"
  section, a stale orphan JSON fragment in `list_backends`, a duplicate
...
- `docs/metrics/features.md`: add missing ADM options (`adm_dlm_weight`, `adm_skip_aim`,
  `adm_skip_scale0`, `adm_min_val`) to the ADM Options table; fix pre-existing errors in
  the table (`adm_ref_display_height` alias corrected from `rdf` to `rdh`, `adm_csf_mode`
...
Corrected status column in `docs/adr/README.md` for 9 ADRs whose index
rows still said **Proposed** after their implementation landed and the
corresponding `docs/adr/NNNN-*.md` files were updated to **Accepted**.
ADRs affected: 0251, 0331, 0341, 0353, 0365, 0394, 0396, 0397, 0403.
...
Tiny-AI frame loading now supports packed colour FFmpeg pixel formats
(`rgb24`, `bgr24`, `rgba`, `bgra`) as `HxWxC` tensors instead of rejecting
everything except grayscale.
...
Fix missing `#include <stddef.h>` in `core/src/feature/x86/cambi_avx512.c`;
...
- **`vmaf` CLI assertion crash on bad `--threads` / `--subsample` /
  ...
  `core/test/fuzz/cli_parse_known_crashes/cli_threads_abbrev_assert.argv`
  ...
  `core/test/test_cli_parse_long_only_args.c` drives `cli_parse`
...
- Fixed two master-side CI breaks blocking every open PR:
  ...
  - `pthread_once_t` use in `core/src/feature/integer_adm.h:45` (added
...
are intact in `core/src/cuda/kernel_template.h` and compile cleanly. The container
...
`g_hip_features[]` routing table in `core/src/hip/dispatch_strategy.c`
...
`"float_ms_ssim"` entry from `core/src/metal/dispatch_strategy.c` that
...
The DNN C API docs now describe the current execution-provider selectors
(CUDA, OpenVINO CPU/GPU/NPU, ROCm, CoreML variants) and the live `fp16_io`
...
Removed stale stub/scaffold labels from public docs whose implementation status
is already accepted, proposed, or explicitly deferred, including the
`vmaf-tune --resolution-aware` usage page.
...
rename. `core/src/vulkan/meson.build` now references only `float_moment_vulkan.c`.
...
**docs/metrics/features.md** — corrected `float_moment` backend list from
"scalar only" to "CUDA, SYCL, Vulkan" (all three GPU implementations shipped
in T7-23; the per-extractor body text had not been updated to match the
overview table).
...
- `docs/backends/hip/overview.md`: update stale "8 of 11" extractor count to
  "20 of 23"; add the 12 extractors added after batch-4 (`integer_psnr_hvs_hip`,
  `integer_cambi_hip`, `ssimulacra2_hip`, `integer_vif_hip`, `integer_motion_hip`,
...
`vmaf_metal_dispatch_supports()` now reports support for the eight
landed Metal feature extractors and their provided feature keys instead
...
fix(sycl): add `enable_lcs` and `enable_chroma` to `float_ms_ssim` SYCL twin (ADR-0526)
...
- **`picture_compute_geometry` off-by-one for odd-height / odd-width YUV
  ...
  `picture_compute_geometry` in `core/src/picture.c` computed chroma plane
  ...
  copies in `core/src/cuda/picture_cuda.c`
  (`vmaf_cuda_picture_alloc_pinned` and `vmaf_cuda_picture_alloc`) and to the
  local subsampling calculations in
  `core/src/feature/cuda/integer_psnr_cuda.c` and
  `core/src/feature/cuda/integer_psnr_hvs_cuda.c`. The `min_sse`
  psnr_max divisor in `core/src/feature/integer_psnr.c` is also corrected.
  ...
  in `core/test/test_picture.c` asserts `pic.w[1] == 289`, `pic.h[1] == 162`
...
- Research digest first-line headers normalized to `# Research-NNNN: <title>`
  ...
  with no Research prefix). Header numbers now match filenames in every case.
...
Restore `vmaf_fex_integer_ms_ssim_hip` (ADR-0285) and `vmaf_fex_integer_vif_metal`
(ADR-0436) extern declarations, `feature_extractor_list[]` entries, and meson
source wire-ins that PR #1067 (Vulkan rename) dropped from master.
...
- **`set_option_double` in `core/src/opt.c` silently accepted `NaN`
  ...
  added to `core/test/test_opt.c`. Surfaced by round-8 bug-hunt,
...
Retire the stale sanitizer deselect for `test_cli_parse` after re-verifying the
test binary under ASan+LSan, UBSan, and TSan.


Retire the stale sanitizer deselect for `test_feature_collector` after
re-verifying it clean under ASan+LSan, UBSan, and TSan.
...
Retire the stale sanitizer deselect for `test_pic_preallocation` after
re-verifying it clean under ASan+LSan, UBSan, and TSan.
...
- **`vmaf --feature ssim` could not resolve.** The fixed-point SSIM
  ...
  `core/src/feature/integer_ssim.c` but the source file was not
  listed in `core/src/meson.build`, and the symbol was not
  ...
  `core/test/test_feature_extractor.c` asserts both that the
  ...
  (`core/src/feature/vulkan/ssim_vulkan.c`) defines
...
- `0546-codec-adapter-two-pass-real.md` → `0595-codec-adapter-two-pass-real.md`


- `docs/state.md`: remove 7 stale or duplicate Open rows (T-PYTHON-COMPARE-NO-BACKEND-PRECHECK,
  T-PYTHON-PERMUTATION-IMPORTANCE-HARDCODED-PATH, T-PYTHON-TRAIN-TEST-STD-ZERO,
  T-PYTHON-ROUTINE-SWALLOWED-EXCEPTION, T-PYTHON-LOCAL-EXPLAINER-HACKY already in
  Recently closed; duplicate T-VK-1.4-BUMP + T-VK-CIEDE-F32-F64 rows). Add 7 new Open rows
  cross-linking in-flight PRs #181 (conflict markers), #213 (legacy-runner stub), #214
  (filter1d dispatch conflict), #215 (read_json_model cpp23), #216 (doc compliance ×2),
  and #217 (motion SAD batching).
...
preallocation so it no longer describes the live
`vmaf_sycl_preallocate_pictures()` pool as a no-op stub.
...
- `vmaf-tune compare` now binds the real Phase-B bisect backend from
  source geometry flags instead of emitting only placeholder failure rows.
...
`vmaf-tune ladder --spacing vmaf` is now accepted by the CLI, and the
legacy `uniform` spelling remains supported as an alias for perceptual
VMAF-spacing instead of failing inside the ladder library.


Made `vmaf-tune tune-per-shot` run the real Phase-B bisect backend per
detected shot instead of assigning every shot the codec adapter's default CRF.


`vmaf-tune` predictor training now consumes directory corpus inputs in
the top-level trainer and CLI paths instead of silently falling back to
synthetic-stub rows.
...
Retired stale `vmaf-tune` usage-doc stub labels for coarse-to-fine search,
Phase E ladder generation, the ladder default sampler, saliency-aware encode,
and fast recommend; the pages now describe the shipped implementations and
...
`core/test/test_output.c` drive them through CSV output with a temp-file path,
