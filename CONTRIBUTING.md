...
every file states it in an `SPDX-License-Identifier` tag. See
[ADR-1250](docs/adr/1250-eupl-fork-relicense.md) for the decision and
[the README](README.md#attribution--license) for what it means downstream.
...
- **A file inherited from or derived from [Netflix/vmaf](https://github.com/Netflix/vmaf)**
  — it keeps [BSD-2-Clause-Patent](LICENSE) and so does your change to it. Do not
  retag such a file, and keep its existing copyright notice.

If you are unsure which case you are in, read the tag already in the file; if the
file has none, ask in the pull request rather than guessing.
...
CUDA sources; `reuse lint` checks that every identifier used in the tree resolves
to a licence text under `LICENSES/`.
